namespace Test.Shared.Suites.Database
{
    ...
    /// <summary>
    /// Descriptors for event database operations: create with full correlation metadata, recent
    /// enumeration with descending ordering, filtered enumeration by type, entity, captain,
    /// mission, vessel, and voyage, and paginated enumeration with type/captain filters. Includes
    /// a negative case for recent enumeration over an empty store. Each case runs against its own
    /// fresh SQLite store.
    /// </summary>
    public sealed class EventSuite : IArmadaTestSuite
    {
        ...
        private const string SuiteId = "Database.Event";
        ...
        /// <summary>
        /// Build the descriptor for the Event Tests suite.
        /// </summary>
        ...
        public TestSuiteDescriptor Build()
        {
            ...
            cases.Add(CaseAsync("event_create", "Event_Create", TestTags.Positive, async () =>
            {
            ...
            cases.Add(CaseAsync("event_enumerate_recent", "Event_EnumerateRecent", TestTags.Positive, async () =>
            {
                using (TestDatabase testDb = await TestDatabaseHelper.CreateDatabaseAsync())
                {
                    SqliteDatabaseDriver db = testDb.Driver;
            ...
            cases.Add(CaseAsync("event_enumerate_by_type", "Event_EnumerateByType", TestTags.Positive, async () =>
            {
                using (TestDatabase testDb = await TestDatabaseHelper.CreateDatabaseAsync())
                {
                    SqliteDatabaseDriver db = testDb.Driver;
            ...
            cases.Add(CaseAsync("event_enumerate_by_entity", "Event_EnumerateByEntity", TestTags.Positive, async () =>
            {
                using (TestDatabase testDb = await TestDatabaseHelper.CreateDatabaseAsync())
                {
                    SqliteDatabaseDriver db = testDb.Driver;
            ...
            cases.Add(CaseAsync("event_enumerate_by_captain", "Event_EnumerateByCaptain", TestTags.Positive, async () =>
            {
                using (TestDatabase testDb = await TestDatabaseHelper.CreateDatabaseAsync())
                {
                    SqliteDatabaseDriver db = testDb.Driver;
            ...
            cases.Add(CaseAsync("event_enumerate_by_mission", "Event_EnumerateByMission", TestTags.Positive, async () =>
            {
                using (TestDatabase testDb = await TestDatabaseHelper.CreateDatabaseAsync())
                {
                    SqliteDatabaseDriver db = testDb.Driver;
            ...
            cases.Add(CaseAsync("event_enumerate_by_vessel", "Event_EnumerateByVessel", TestTags.Positive, async () =>
            {
                using (TestDatabase testDb = await TestDatabaseHelper.CreateDatabaseAsync())
                {
                    SqliteDatabaseDriver db = testDb.Driver;
            ...
            cases.Add(CaseAsync("event_enumerate_by_voyage", "Event_EnumerateByVoyage", TestTags.Positive, async () =>
            {
                using (TestDatabase testDb = await TestDatabaseHelper.CreateDatabaseAsync())
                {
                    SqliteDatabaseDriver db = testDb.Driver;
            ...
            cases.Add(CaseAsync("event_enumerate_paginated", "Event_EnumeratePaginated", TestTags.Positive, async () =>
            {
                using (TestDatabase testDb = await TestDatabaseHelper.CreateDatabaseAsync())
                {
                    SqliteDatabaseDriver db = testDb.Driver;
            ...
            // Audit addition: recent enumeration over an empty store must return no rows.
            cases.Add(CaseAsync("event_enumerate_recent_empty_returns_empty", "Event_EnumerateRecent_EmptyReturnsEmpty", TestTags.Negative, async () =>
            {
                using (TestDatabase testDb = await TestDatabaseHelper.CreateDatabaseAsync())
                {
                    SqliteDatabaseDriver db = testDb.Driver;
