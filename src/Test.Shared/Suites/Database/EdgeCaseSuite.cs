namespace Test.Shared.Suites.Database
{
    ...
    public sealed class EdgeCaseSuite : IArmadaTestSuite
    {
        ...
        /// <summary>
        /// Build the descriptor for the Edge Cases suite.
        /// </summary>
        ...
        public TestSuiteDescriptor Build()
        {
            ...
            cases.Add(CaseAsync("pagination_page_zero_clamps_to_one", "EdgeCase_PaginationPageZero", TestTags.Negative, async () =>
            {
            ...
            cases.Add(CaseAsync("order_ascending", "EdgeCase_OrderAscending", TestTags.Positive, async () =>
            {
                using (TestDatabase testDb = await TestDatabaseHelper.CreateDatabaseAsync())
                {
                    SqliteDatabaseDriver db = testDb.Driver;
            ...
            cases.Add(CaseAsync("order_descending", "EdgeCase_OrderDescending", TestTags.Positive, async () =>
            {
                using (TestDatabase testDb = await TestDatabaseHelper.CreateDatabaseAsync())
                {
                    SqliteDatabaseDriver db = testDb.Driver;
            ...
            cases.Add(CaseAsync("empty_result_set", "EdgeCase_EmptyResultSet", TestTags.Negative, async () =>
            {
                using (TestDatabase testDb = await TestDatabaseHelper.CreateDatabaseAsync())
                {
                    SqliteDatabaseDriver db = testDb.Driver;
            ...
            cases.Add(CaseAsync("unicode_text", "EdgeCase_UnicodeText", TestTags.Positive, async () =>
            {
                using (TestDatabase testDb = await TestDatabaseHelper.CreateDatabaseAsync())
                {
                    SqliteDatabaseDriver db = testDb.Driver;
            ...
            cases.Add(CaseAsync("null_optional_fields", "EdgeCase_NullOptionalFields", TestTags.Negative, async () =>
            {
                using (TestDatabase testDb = await TestDatabaseHelper.CreateDatabaseAsync())
                {
                    SqliteDatabaseDriver db = testDb.Driver;
            ...
            cases.Add(CaseAsync("concurrent_try_claim", "EdgeCase_ConcurrentTryClaim", TestTags.Negative, async () =>
            {
                using (TestDatabase testDb = await TestDatabaseHelper.CreateDatabaseAsync())
                {
                    SqliteDatabaseDriver db = testDb.Driver;
            ...
            cases.Add(CaseAsync("large_enumeration", "EdgeCase_LargeEnumeration", TestTags.Positive, async () =>
            {
                using (TestDatabase testDb = await TestDatabaseHelper.CreateDatabaseAsync())
                {
                    SqliteDatabaseDriver db = testDb.Driver;
            ...
            // Audit addition: deleting a non-existent fleet id is a no-op that leaves existing rows intact.
            cases.Add(CaseAsync("delete_nonexistent_is_noop_audit", "EdgeCase_DeleteNonExistentIsNoop", TestTags.Negative, async () =>
            {
                using (TestDatabase testDb = await TestDatabaseHelper.CreateDatabaseAsync())
                {
                    SqliteDatabaseDriver db = testDb.Driver;
                    ...
                    List<Fleet> remaining = await db.Fleets.EnumerateAsync();
                    AssertEqual(1, remaining.Count, "Deleting a non-existent fleet should not affect existing rows");
                }
            }));

            return new TestSuiteDescriptor(
                suiteId: "Database.EdgeCase",
