namespace Armada.Helm.Commands
{
    ...
    /// </summary>
    [Description("Configure MCP integration for Claude Code, Codex, Gemini, Cursor, Mux, and OpenCode (when detected)")]
    public class McpInstallCommand : BaseCommand<McpInstallSettings>
    {
        ...
        protected override async Task<int> ExecuteAsync(CommandContext context, McpInstallSettings settings, CancellationToken cancellationToken)
        {
            ...
            string mcpHttpUrl = McpConfigHelper.GetMcpUrl(armadaSettings.McpPort);
            ...
            AnsiConsole.MarkupLine($"[dim]MCP endpoint:[/] [green]{Markup.Escape(mcpHttpUrl)}[/]");
