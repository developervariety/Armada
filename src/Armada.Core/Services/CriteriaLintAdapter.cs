    ...
    public sealed class CriteriaLintAdapter
    {
        ...
        private static readonly IReadOnlyList<CriterionNoul> _Nouls = new List<CriterionNoul>
        {
            ...
            new CriterionNoul("pins_total",
                "Meeting this criterion depends on a test pass total, skip total, or coverage figure staying at a fixed number — a value that legitimately varies between host and dock — so an unchanged, healthy suite could read as failing it. Only a failure count (zero failed) is a stable contract.",
                "a healthy run could fail this criterion because a legitimately varying count moved",
                "the criterion holds for any healthy run, depending on no such varying count"),
