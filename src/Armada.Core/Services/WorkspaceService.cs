    ...
    public class WorkspaceService : IWorkspaceService
    {
        ...
        public async Task<WorkspaceDiffResult> GetDiffAsync(Vessel vessel, string? path = null, CancellationToken token = default)
        {
            ...
            try
            {
                ...
                // `--show-prefix` is used instead of comparing `--show-toplevel` against the requested
                // root, because git resolves symlinks (on macOS /var -> /private/var) while
                // Path.GetFullPath does not, so a string comparison rejects a legitimate root.
                string prefix;
                try
                {
                    prefix = (await RunGitCommandAsync(rootPath, token, "rev-parse", "--show-prefix").ConfigureAwait(false)).Trim();
                }
                catch
                {
