    ...
    public sealed class PreflightTextAdapter
    {
        ...
        private static readonly IReadOnlyList<PreflightNoul> _Nouls = new List<PreflightNoul>
        {
            ...
            new PreflightNoul("q8", 8,
                "A pinned number is not a count measured at a named commit, or a pass or skip total is pinned as a contract.",
                "a pinned number is not a measured count", "every pinned number is a measured count"),
            ...
            CancellationToken token)
        {
            ...
            try
            {
                object state = DecisionStateRedactor.RedactObject(BuildState(objective, vessel, pipeline, result), _Settings.MaxStateChars);
