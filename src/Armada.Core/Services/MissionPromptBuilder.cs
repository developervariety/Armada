        ...
        internal static string GetPersonaOutputContract(string? persona, MissionModeEnum mode, string? judgePrimaryLens)
        {
            ...
            return PersonaCatalog.NormalizeName(persona) switch
            {
                ...
                PersonaCatalog.Judge =>
                    "Your response must contain these exact section headings: " + JudgeReviewSections.Headings(false) + ". When the brief lists acceptance criteria, also include `## Acceptance Criteria` with one MET or NOT MET line per criterion; a NOT MET line forbids PASS. When the mission description contains [DOD:DOC-ONLY] and the diff is non-code, do not run the full suite. Do not reply with only a verdict line or brief summary. Run the test suite in the FOREGROUND and wait for it to finish before reaching a verdict -- never launch tests as a background task and schedule a wakeup, and never terminate before the verdict is emitted, except for [DOD:DOC-ONLY] non-code diffs. Emit your verdict synchronously: the very last thing you do must be to print exactly one standalone line `[ARMADA:VERDICT] PASS`, `[ARMADA:VERDICT] FAIL`, or `[ARMADA:VERDICT] NEEDS_REVISION`. Before you exit, verify that your response already contains the standalone verdict line. If it does not, emit it immediately. A review without that line is discarded and re-run." + BuildJudgeLensDirective(judgePrimaryLens),
                _ => String.Empty
            };
        }
