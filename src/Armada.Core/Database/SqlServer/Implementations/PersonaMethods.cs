        ...
        public async Task<Persona> CreateAsync(Persona persona, CancellationToken token = default)
        {
            ...
            using (SqlConnection conn = new SqlConnection(_Driver.ConnectionString))
            {
                ...
                using (SqlCommand cmd = conn.CreateCommand())
                {
                    ...
                    PersonaColumns.Write(SqlServerDatabaseDriver.StoredBinder.For(cmd, "personas"), persona);
                ...
                using (SqlCommand cmd = conn.CreateCommand())
                {
                    ...
                    using (SqlDataReader reader = await cmd.ExecuteReaderAsync(token).ConfigureAwait(false))
                    {
                        if (await reader.ReadAsync(token).ConfigureAwait(false))
                            return PersonaColumns.Read(reader, SqlServerDatabaseDriver.StoredValues);
                    }
                }
            }
                ...
                using (SqlCommand cmd = conn.CreateCommand())
                {
                    ...
                    using (SqlDataReader reader = await cmd.ExecuteReaderAsync(token).ConfigureAwait(false))
                    {
                        while (await reader.ReadAsync(token).ConfigureAwait(false))
                            results.Add(PersonaColumns.Read(reader, SqlServerDatabaseDriver.StoredValues));
                    }
                }
            }
        ...
        public async Task<EnumerationResult<Persona>> EnumerateAsync(EnumerationQuery query, CancellationToken token = default)
        {
            ...
            using (SqlConnection conn = new SqlConnection(_Driver.ConnectionString))
            {
                ...
                if (query.CreatedAfter.HasValue)
                {
                    ...
                    parameters.Add(SqlServerDatabaseDriver.StoredBinder.Timestamp(new SqlParameter(), "@created_after", "personas", "created_utc", query.CreatedAfter.Value));
                }
                if (query.CreatedBefore.HasValue)
                {
                    conditions.Add("created_utc < @created_before");
                    parameters.Add(SqlServerDatabaseDriver.StoredBinder.Timestamp(new SqlParameter(), "@created_before", "personas", "created_utc", query.CreatedBefore.Value));
                }
