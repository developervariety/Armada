namespace Armada.Core.Database.SqlServer.Implementations
{
    ...
    /// <summary>
    /// SQL Server implementation of planning session persistence.
    /// </summary>
    public class PlanningSessionMethods : IPlanningSessionMethods
    {
        ...
        private async Task<PlanningSession?> ReadInternalAsync(string sql, Action<SqlCommand> parameterize, CancellationToken token)
        {
            using (SqlConnection conn = new SqlConnection(_Driver.ConnectionString))
            {
                await conn.OpenAsync(token).ConfigureAwait(false);
        ...
        private async Task<List<PlanningSession>> EnumerateInternalAsync(string sql, Action<SqlCommand>? parameterize, CancellationToken token)
        {
