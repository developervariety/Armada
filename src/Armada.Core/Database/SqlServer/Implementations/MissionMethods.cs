...
#pragma warning restore CS0414
        ...
        public async Task<Mission> CreateAsync(Mission mission, CancellationToken token = default)
        {
            ...
            using (SqlConnection conn = new SqlConnection(_Driver.ConnectionString))
            {
                ...
                using (SqlCommand cmd = conn.CreateCommand())
                {
                    ...
                    cmd.Parameters.AddWithValue("@reconciled_utc", mission.ReconciledUtc.HasValue ? (object)SqlServerDatabaseDriver.ToIso8601(mission.ReconciledUtc.Value) : DBNull.Value);
        ...
        public async Task<Mission> UpdateAsync(Mission mission, CancellationToken token = default)
        {
            ...
            using (SqlConnection conn = new SqlConnection(_Driver.ConnectionString))
            {
                ...
                using (SqlCommand cmd = conn.CreateCommand())
                {
                    cmd.CommandText = "UPDATE missions SET " + MissionAdmissionPersistence.PreserveOwnerSql(DatabaseTypeEnum.SqlServer) + @", tier = @tier, requested_captain_id = @requested_captain_id,
                        tenant_id = @tenant_id,
                            user_id = @user_id,
                        voyage_id = @voyage_id,
                        vessel_id = @vessel_id,
