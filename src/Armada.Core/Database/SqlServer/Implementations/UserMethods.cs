        ...
        public async Task<UserMaster> CreateAsync(UserMaster user, CancellationToken token = default)
        {
            ...
            using (SqlConnection conn = new SqlConnection(_Driver.ConnectionString))
            {
                ...
                using (SqlCommand cmd = conn.CreateCommand())
                {
                    ...
                    UserColumns.Write(SqlServerDatabaseDriver.StoredBinder.For(cmd, "users"), user);
                ...
                using (SqlCommand cmd = conn.CreateCommand())
                {
                    ...
                    using (SqlDataReader reader = await cmd.ExecuteReaderAsync(token).ConfigureAwait(false))
                    {
                        if (await reader.ReadAsync(token).ConfigureAwait(false))
                            return UserColumns.Read(reader, SqlServerDatabaseDriver.StoredValues);
                    }
                }
            }
                ...
                using (SqlCommand cmd = conn.CreateCommand())
                {
                    ...
                    using (SqlDataReader reader = await cmd.ExecuteReaderAsync(token).ConfigureAwait(false))
                    {
                        while (await reader.ReadAsync(token).ConfigureAwait(false))
                            results.Add(UserColumns.Read(reader, SqlServerDatabaseDriver.StoredValues));
                    }
                }
            }
        ...
        public async Task<EnumerationResult<UserMaster>> EnumerateAsync(string tenantId, EnumerationQuery query, CancellationToken token = default)
        {
            ...
            using (SqlConnection conn = new SqlConnection(_Driver.ConnectionString))
            {
                ...
                if (query.CreatedAfter.HasValue)
                {
                    ...
                    parameters.Add(SqlServerDatabaseDriver.StoredBinder.Timestamp(new SqlParameter(), "@created_after", "users", "created_utc", query.CreatedAfter.Value));
                }
                if (query.CreatedBefore.HasValue)
                {
                    conditions.Add("created_utc < @created_before");
                    parameters.Add(SqlServerDatabaseDriver.StoredBinder.Timestamp(new SqlParameter(), "@created_before", "users", "created_utc", query.CreatedBefore.Value));
                }
