namespace Armada.Core.Database.SqlServer.Implementations
{
    ...
    public class RequestHistoryMethods : IRequestHistoryMethods
    {
        ...
        {
            RequestHistoryColumns.WriteEntry(SqlServerDatabaseDriver.StoredBinder.For(cmd, "request_history"), entry);
        }

        private static void BindDetail(SqlCommand cmd, RequestHistoryDetail detail)
        {
            RequestHistoryColumns.WriteDetail(SqlServerDatabaseDriver.StoredBinder.For(cmd, "request_history_details"), detail);
        }
        ...
        private static void ApplyQueryFilters(RequestHistoryQuery? query, List<string> conditions, List<SqlParameter> parameters)
        {
            ...
            if (!string.IsNullOrWhiteSpace(query.CredentialId))
            {
                ...
                parameters.Add(StoredValueBinder.Parameter(new SqlParameter(), "@credential_id", query.CredentialId));
            }
            if (!string.IsNullOrWhiteSpace(query.Principal))
            {
                conditions.Add("principal_display LIKE @principal");
                parameters.Add(StoredValueBinder.Parameter(new SqlParameter(), "@principal", "%" + query.Principal + "%"));
            }
            if (!string.IsNullOrWhiteSpace(query.Method))
            {
                conditions.Add("UPPER(method) = @method");
                parameters.Add(StoredValueBinder.Parameter(new SqlParameter(), "@method", query.Method.ToUpperInvariant()));
            }
            if (!string.IsNullOrWhiteSpace(query.Route))
            {
                conditions.Add("route LIKE @route");
                parameters.Add(StoredValueBinder.Parameter(new SqlParameter(), "@route", "%" + query.Route + "%"));
            }
            if (query.StatusCode.HasValue)
            {
                conditions.Add("status_code = @status_code");
                parameters.Add(StoredValueBinder.Parameter(new SqlParameter(), "@status_code", query.StatusCode.Value));
            }
            if (query.IsSuccess.HasValue)
            {
                conditions.Add("is_success = @is_success");
                parameters.Add(SqlServerDatabaseDriver.StoredBinder.Boolean(new SqlParameter(), "@is_success", "request_history", "is_success", query.IsSuccess.Value));
            }
            if (query.FromUtc.HasValue)
            {
                conditions.Add("created_utc >= @from_utc");
                parameters.Add(SqlServerDatabaseDriver.StoredBinder.Timestamp(new SqlParameter(), "@from_utc", "request_history", "created_utc", query.FromUtc.Value));
            }
            if (query.ToUtc.HasValue)
            {
                conditions.Add("created_utc <= @to_utc");
                parameters.Add(SqlServerDatabaseDriver.StoredBinder.Timestamp(new SqlParameter(), "@to_utc", "request_history", "created_utc", query.ToUtc.Value));
            }
        }
    }
}
