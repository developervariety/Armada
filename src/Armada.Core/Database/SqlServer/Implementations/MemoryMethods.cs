namespace Armada.Core.Database.SqlServer.Implementations
{
    ...
    /// <summary>
    /// SQL Server implementation of native captain memory persistence, including the memory_tags child table.
    /// </summary>
    public class MemoryMethods : IMemoryMethods
    {
        ...
        public async Task<bool> DeleteAsync(string tenantId, string id, CancellationToken token = default)
        {
            ...
            using (SqlConnection conn = new SqlConnection(_Driver.ConnectionString))
            {
                ...
                using (SqlTransaction tx = (SqlTransaction)await conn.BeginTransactionAsync(token).ConfigureAwait(false))
                {
                    ...
                    {
                        cmd.Transaction = tx;
                        cmd.CommandText = "DELETE FROM memory_tags WHERE memory_id = @id AND EXISTS (SELECT 1 FROM memories WHERE id = @id AND tenant_id = @tenant_id);";
                        StoredValueBinder.Value(cmd, "@tenant_id", tenantId);
                        StoredValueBinder.Value(cmd, "@id", id);
                        await cmd.ExecuteNonQueryAsync(token).ConfigureAwait(false);
                    }
        ...
        private async Task<List<Memory>> QueryAsync(string memorySql, string tagSql, Action<SqlCommand>? parameterize, CancellationToken token)
        {
        ...
        private static async Task WriteTagsAsync(SqlConnection conn, SqlTransaction tx, Memory memory, CancellationToken token)
        {
        ...
        private static void Bind(SqlCommand cmd, Memory memory)
        {
            MemoryColumns.Write(SqlServerDatabaseDriver.StoredBinder.For(cmd, "memories"), memory);
        }
