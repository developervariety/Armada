...
CHAR_LENGTH(COALESCE(agent_output, '')) AS agent_output_length";
            ...
            CancellationToken token)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(_ConnectionString))
            {
                ...
                using (MySqlCommand cmd = conn.CreateCommand())
                {
                    ...
                    AddParameters(cmd, baseParameters);
                    MysqlDatabaseDriver.StoredBinder.For(cmd, "missions").Utc("@from_utc", "created_utc", query.FromUtc);
                    MysqlDatabaseDriver.StoredBinder.For(cmd, "missions").Utc("@to_utc", "created_utc", query.ToUtc);
        ...
        private static void ApplyEnumerationFilters(EnumerationQuery query, List<string> conditions, MySqlCommand cmd)
        {
            if (query.CreatedAfter.HasValue)
            {
                ...
                MysqlDatabaseDriver.StoredBinder.For(cmd, "missions").Utc("@created_after", "created_utc", query.CreatedAfter.Value);
            }
            if (query.CreatedBefore.HasValue)
            {
                conditions.Add("created_utc < @created_before");
                MysqlDatabaseDriver.StoredBinder.For(cmd, "missions").Utc("@created_before", "created_utc", query.CreatedBefore.Value);
            }
