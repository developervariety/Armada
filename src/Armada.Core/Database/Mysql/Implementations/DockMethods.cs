    ...
    public partial class DockMethods : IDockMethods
    {
        ...
        public async Task<Dock> CreateAsync(Dock dock, CancellationToken token = default)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(_ConnectionString))
            {
                ...
                using (MySqlCommand cmd = conn.CreateCommand())
                {
                    ...
                    DockColumns.Write(MysqlDatabaseDriver.StoredBinder.For(cmd, "docks"), dock);
        ...
        public async Task<Dock> UpdateAsync(Dock dock, CancellationToken token = default)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(_ConnectionString))
            {
                ...
                using (MySqlCommand cmd = conn.CreateCommand())
                {
                    cmd.CommandText = @"UPDATE docks SET " + DockGitAnchorUpdate.PreserveOnSameOwnerSql(DatabaseTypeEnum.Mysql) + @",
                        tenant_id = @tenant_id,
                            user_id = @user_id,
                        vessel_id = @vessel_id,
        ...
        public async Task<EnumerationResult<Dock>> EnumerateAsync(EnumerationQuery query, CancellationToken token = default)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(_ConnectionString))
            {
                ...
                if (query.CreatedAfter.HasValue)
                {
                    ...
                    parameters.Add(MysqlDatabaseDriver.StoredBinder.Timestamp(new MySqlParameter(), "@created_after", "docks", "created_utc", query.CreatedAfter.Value));
                }
                if (query.CreatedBefore.HasValue)
                {
                    conditions.Add("created_utc < @created_before");
                    parameters.Add(MysqlDatabaseDriver.StoredBinder.Timestamp(new MySqlParameter(), "@created_before", "docks", "created_utc", query.CreatedBefore.Value));
                }
