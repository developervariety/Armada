        ...
        public async Task<Captain> CreateAsync(Captain captain, CancellationToken token = default)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(_ConnectionString))
            {
                ...
                using (MySqlCommand cmd = conn.CreateCommand())
                {
                    ...
                    cmd.Parameters.AddWithValue("@process_started_utc", captain.ProcessStartedUtc.HasValue ? (object)ToDatabaseTimestamp(captain.ProcessStartedUtc.Value) : DBNull.Value);
        ...
        public async Task<bool> TryQuarantineIdleAsync(string captainId, string reason, DateTime? untilUtc, CancellationToken token = default, bool preserveStrongerHold = false)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(_ConnectionString))
            {
                ...
                using (MySqlCommand cmd = conn.CreateCommand())
                {
                    ...
                    cmd.Parameters.AddWithValue("@quarantine_until_utc", untilUtc.HasValue ? (object)ToDatabaseTimestamp(untilUtc.Value) : DBNull.Value);
            ...
            using (MySqlConnection conn = new MySqlConnection(_ConnectionString))
            {
                ...
                using (MySqlCommand cmd = conn.CreateCommand())
                {
                    ...
                    if (expiredAtOrBeforeUtc.HasValue)
                        cmd.Parameters.AddWithValue("@cutoff", ToDatabaseTimestamp(expiredAtOrBeforeUtc.Value));
                    int rowsAffected = await cmd.ExecuteNonQueryAsync(token).ConfigureAwait(false);
                    return rowsAffected > 0;
                }
            }
