namespace Armada.Core.Database.Mysql.Implementations
{
    ...
    /// <summary>
    /// MySQL implementation of playbook persistence and associations.
    /// </summary>
    public class PlaybookMethods : IPlaybookMethods
    {
        ...
        /// Instantiate.
        /// </summary>
        public PlaybookMethods(string connectionString)
        {
        ...
        public async Task SetVoyageSelectionsAsync(string voyageId, List<SelectedPlaybook> selections, CancellationToken token = default)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(_ConnectionString))
            {
                ...
                using (MySqlTransaction tx = await conn.BeginTransactionAsync(token).ConfigureAwait(false))
                {
                    using (MySqlCommand deleteCmd = conn.CreateCommand())
                    {
        ...
        private async Task<EnumerationResult<Playbook>> EnumerateInternalAsync(string? tenantId, EnumerationQuery query, CancellationToken token)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(_ConnectionString))
            {
                ...
                using (MySqlCommand countCmd = conn.CreateCommand())
                {
                    ...
                    foreach (MySqlParameter p in parameters) countCmd.Parameters.Add(new MySqlParameter(p.ParameterName, p.Value));
        ...
        private static async Task DeleteMissionSnapshotsByPlaybookAsync(MySqlConnection conn, MySqlTransaction tx, string playbookId, CancellationToken token)
        {
            using (MySqlCommand cmd = conn.CreateCommand())
            {
                cmd.Transaction = tx;
