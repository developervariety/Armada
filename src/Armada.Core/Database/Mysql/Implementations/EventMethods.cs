        ...
        public async Task<ArmadaEvent> CreateAsync(ArmadaEvent armadaEvent, CancellationToken token = default)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(_ConnectionString))
            {
                ...
                using (MySqlCommand cmd = conn.CreateCommand())
                {
                    ...
                    EventColumns.Write(MysqlDatabaseDriver.StoredBinder.For(cmd, "events"), armadaEvent);
        ...
        public async Task<EnumerationResult<ArmadaEvent>> EnumerateAsync(EnumerationQuery query, CancellationToken token = default)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(_ConnectionString))
            {
                ...
                if (query.CreatedAfter.HasValue)
                {
                    ...
                    parameters.Add(MysqlDatabaseDriver.StoredBinder.Timestamp(new MySqlParameter(), "@created_after", "events", "created_utc", query.CreatedAfter.Value));
                }
                if (query.CreatedBefore.HasValue)
                {
                    conditions.Add("created_utc < @created_before");
                    parameters.Add(MysqlDatabaseDriver.StoredBinder.Timestamp(new MySqlParameter(), "@created_before", "events", "created_utc", query.CreatedBefore.Value));
                }
                if (!string.IsNullOrEmpty(query.EventType))
                {
                    conditions.Add("event_type = @event_type");
                    parameters.Add(StoredValueBinder.Parameter(new MySqlParameter(), "@event_type", query.EventType));
                }
