...
LENGTH(COALESCE(agent_output, '')) AS agent_output_length";
            ...
            CancellationToken token)
        {
            ...
            using (NpgsqlConnection conn = new NpgsqlConnection(_Settings.GetConnectionString()))
            {
                ...
                using (NpgsqlCommand cmd = new NpgsqlCommand())
                {
                    ...
                    AddParameters(cmd, baseParameters);
                    PostgresqlDatabaseDriver.StoredBinder.For(cmd, "missions").Utc("@from_utc", "created_utc", query.FromUtc);
                    PostgresqlDatabaseDriver.StoredBinder.For(cmd, "missions").Utc("@to_utc", "created_utc", query.ToUtc);
        ...
        private static void ApplyEnumerationFilters(EnumerationQuery query, List<string> conditions, NpgsqlCommand cmd)
        {
            if (query.CreatedAfter.HasValue)
            {
                ...
                PostgresqlDatabaseDriver.StoredBinder.For(cmd, "missions").Utc("@created_after", "created_utc", query.CreatedAfter.Value);
            }
            if (query.CreatedBefore.HasValue)
            {
                conditions.Add("created_utc < @created_before");
                PostgresqlDatabaseDriver.StoredBinder.For(cmd, "missions").Utc("@created_before", "created_utc", query.CreatedBefore.Value);
            }
