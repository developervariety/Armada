namespace Armada.Core.Database.Postgresql.Implementations
{
    ...
    public sealed class JudgeFollowUpMethods : IJudgeFollowUpMethods
    {
        ...
        public async Task<bool> TryAssociateAsync(string followUpId, string mergeEntryId, CancellationToken token = default)
        {
            ...
            using (NpgsqlConnection conn = new NpgsqlConnection(_Settings.GetConnectionString()))
            {
                ...
                using (NpgsqlCommand cmd = conn.CreateCommand())
                {
                    ...
                    PostgresqlDatabaseDriver.StoredBinder.For(cmd, "judge_follow_ups").Utc("@last_update_utc", "last_update_utc", DateTime.UtcNow);
        ...
        public async Task<JudgeFollowUp> CompleteAuditAsync(string id, string verdict, string notes, string? recommendedAction, DateTime completedUtc, CancellationToken token = default)
        {
            ...
            using (NpgsqlConnection conn = new NpgsqlConnection(_Settings.GetConnectionString()))
            {
                ...
                using (NpgsqlCommand cmd = conn.CreateCommand())
                {
                    ...
                    PostgresqlDatabaseDriver.StoredBinder.For(cmd, "judge_follow_ups").Utc("@audit_completed_utc", "audit_completed_utc", completedUtc); PostgresqlDatabaseDriver.StoredBinder.For(cmd, "judge_follow_ups").Utc("@last_update_utc", "last_update_utc", DateTime.UtcNow);
                    await cmd.ExecuteNonQueryAsync(token).ConfigureAwait(false);
                }
            }
        ...
        {
            JudgeFollowUpColumns.Write(PostgresqlDatabaseDriver.StoredBinder.For(cmd, "judge_follow_ups"), item);
        }
