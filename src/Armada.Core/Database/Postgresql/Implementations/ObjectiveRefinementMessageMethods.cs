namespace Armada.Core.Database.Postgresql.Implementations
{
    ...
    public class ObjectiveRefinementMessageMethods : IObjectiveRefinementMessageMethods
    {
                ...
                using (NpgsqlCommand cmd = conn.CreateCommand())
                {
                    ...
                    using (NpgsqlDataReader reader = await cmd.ExecuteReaderAsync(token).ConfigureAwait(false))
                    {
                        if (await reader.ReadAsync(token).ConfigureAwait(false))
                            return ObjectiveRefinementMessageColumns.Read(reader, PostgresqlDatabaseDriver.StoredValues);
                    }
                }
            }
                ...
                using (NpgsqlCommand cmd = conn.CreateCommand())
                {
                    ...
                    using (NpgsqlDataReader reader = await cmd.ExecuteReaderAsync(token).ConfigureAwait(false))
                    {
                        while (await reader.ReadAsync(token).ConfigureAwait(false))
                            results.Add(ObjectiveRefinementMessageColumns.Read(reader, PostgresqlDatabaseDriver.StoredValues));
                    }
                }
            }
        ...
        {
            ObjectiveRefinementMessageColumns.Write(PostgresqlDatabaseDriver.StoredBinder.For(cmd, "objective_refinement_messages"), message);
        }
    }
}
