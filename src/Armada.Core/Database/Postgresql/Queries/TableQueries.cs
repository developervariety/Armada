namespace Armada.Core.Database.Postgresql.Queries
{
    ...
    public static class TableQueries
    {
        ...
        /// <summary>
        /// Migration 101: the durable terminal-voyage reconciliation marker on missions. The backfill marks
        ...
        /// </summary>
        public static readonly string[] MigrationV101Statements = new string[]
        {
            @"ALTER TABLE missions ADD COLUMN IF NOT EXISTS reconciled_utc TIMESTAMP NULL;",
            @"ALTER TABLE missions ADD COLUMN IF NOT EXISTS reconciled_reason TEXT NULL;",
            @"UPDATE missions SET reconciled_utc = COALESCE(completed_utc, last_update_utc), reconciled_reason = 'terminal_voyage_work_unlanded' WHERE reconciled_utc IS NULL AND status IN ('Failed', 'Cancelled') AND failure_reason IS NOT NULL AND length((CASE WHEN strpos(failure_reason, '; previous reason: ') > 0 THEN substr(failure_reason, 1, strpos(failure_reason, '; previous reason: ') - 1) ELSE failure_reason END)) > 31 AND right((CASE WHEN strpos(failure_reason, '; previous reason: ') > 0 THEN substr(failure_reason, 1, strpos(failure_reason, '; previous reason: ') - 1) ELSE failure_reason END), 31) = '(terminal_voyage_work_unlanded)';",
            @"UPDATE missions SET reconciled_utc = COALESCE(completed_utc, last_update_utc), reconciled_reason = 'terminal_voyage_commit_absent' WHERE reconciled_utc IS NULL AND status IN ('Failed', 'Cancelled') AND failure_reason IS NOT NULL AND length((CASE WHEN strpos(failure_reason, '; previous reason: ') > 0 THEN substr(failure_reason, 1, strpos(failure_reason, '; previous reason: ') - 1) ELSE failure_reason END)) > 31 AND right((CASE WHEN strpos(failure_reason, '; previous reason: ') > 0 THEN substr(failure_reason, 1, strpos(failure_reason, '; previous reason: ') - 1) ELSE failure_reason END), 31) = '(terminal_voyage_commit_absent)';",
            @"UPDATE missions SET reconciled_utc = COALESCE(completed_utc, last_update_utc), reconciled_reason = 'terminal_voyage_no_commit' WHERE reconciled_utc IS NULL AND status IN ('Failed', 'Cancelled') AND failure_reason IS NOT NULL AND length((CASE WHEN strpos(failure_reason, '; previous reason: ') > 0 THEN substr(failure_reason, 1, strpos(failure_reason, '; previous reason: ') - 1) ELSE failure_reason END)) > 27 AND right((CASE WHEN strpos(failure_reason, '; previous reason: ') > 0 THEN substr(failure_reason, 1, strpos(failure_reason, '; previous reason: ') - 1) ELSE failure_reason END), 27) = '(terminal_voyage_no_commit)';"
        };
        ...
        public static List<SchemaMigration> GetMigrations()
        {
            return new List<SchemaMigration>
            {
                ...
                new SchemaMigration(33, "Add audit columns to merge_entries and calibration counter to vessels",
                    @"ALTER TABLE merge_entries ADD COLUMN IF NOT EXISTS audit_lane TEXT;",
                    @"ALTER TABLE merge_entries ADD COLUMN IF NOT EXISTS audit_convention_passed BOOLEAN;",
                    @"ALTER TABLE merge_entries ADD COLUMN IF NOT EXISTS audit_convention_notes TEXT;",
                    @"ALTER TABLE merge_entries ADD COLUMN IF NOT EXISTS audit_critical_trigger TEXT;",
                    @"ALTER TABLE merge_entries ADD COLUMN IF NOT EXISTS audit_deep_picked BOOLEAN;",
                    @"ALTER TABLE merge_entries ADD COLUMN IF NOT EXISTS audit_deep_completed_utc TEXT;",
                    @"ALTER TABLE merge_entries ADD COLUMN IF NOT EXISTS audit_deep_verdict TEXT;",
                ...
                new SchemaMigration(44, "Add playbook and threshold columns to personas and captains",
                    @"ALTER TABLE personas ADD COLUMN IF NOT EXISTS default_playbooks TEXT;",
                    @"ALTER TABLE personas ADD COLUMN IF NOT EXISTS curate_threshold INTEGER;",
                    @"ALTER TABLE personas ADD COLUMN IF NOT EXISTS learned_playbook_id TEXT;",
                ...
                new SchemaMigration(61, "Add retry_skip_captain_ids column to missions so in-place judge re-runs route to a different captain without consuming the rescue budget",
                    @"ALTER TABLE missions ADD COLUMN IF NOT EXISTS retry_skip_captain_ids TEXT;"
                ),
                new SchemaMigration(62, "Add skills directory",
                ...
                new SchemaMigration(63, "Add reasoning_effort to captains",
                    @"ALTER TABLE captains ADD COLUMN IF NOT EXISTS reasoning_effort TEXT;"
                ),
                new SchemaMigration(64, "Add redispatch_attempts to missions for no-op completion recovery",
                    @"ALTER TABLE missions ADD COLUMN IF NOT EXISTS redispatch_attempts INTEGER NOT NULL DEFAULT 0;"
                ),
                new SchemaMigration(65, "Add dock-boundary scanner config to vessels",
                    @"ALTER TABLE vessels ADD COLUMN IF NOT EXISTS secret_scan_enabled INTEGER NOT NULL DEFAULT 0;",
                ...
                new SchemaMigration(66, "Add jobs table for background job tracking",
                ...
                new SchemaMigration(67, "Add per-step captain selection (persona default captain, mission requested captain, voyage captain overrides)",
                ...
                new SchemaMigration(68, "Add token_usage table for per-model token accounting",
                ...
                new SchemaMigration(69, "Add project profiles for layered persona resolution",
                ...
                new SchemaMigration(94, "Drop the vessel_pack_hints table, threshold and playbook reference columns and the catalog rows they describe", CatalogAndColumnPruneSchema.PostgresqlStatements),
                new SchemaMigration(95, "Persist mission attempt facts",
                    @"CREATE TABLE IF NOT EXISTS mission_attempt_facts (id TEXT PRIMARY KEY, tenant_id TEXT, user_id TEXT, mission_id TEXT NOT NULL, voyage_id TEXT, vessel_id TEXT, root_mission_id TEXT NOT NULL, parent_mission_id TEXT, fact_type TEXT NOT NULL, is_rescue BOOLEAN NOT NULL DEFAULT FALSE, reason_code TEXT, created_utc TIMESTAMPTZ NOT NULL);",
                ...
                new SchemaMigration(96, "Persist Check regression links",
                    @"ALTER TABLE check_runs ADD COLUMN IF NOT EXISTS regression_purpose TEXT;",
                ...
                new SchemaMigration(98, "Persist lane state transitions and Check slot requests",
                    @"ALTER TABLE check_runs ADD COLUMN IF NOT EXISTS slot_requested_utc TIMESTAMPTZ NULL;",
                    @"CREATE TABLE IF NOT EXISTS lane_state_transitions (id TEXT PRIMARY KEY, lane_key TEXT NOT NULL, eligible_count INTEGER NOT NULL, occupied INTEGER NOT NULL, capacity INTEGER NOT NULL, block_reason TEXT NOT NULL, eligible_source_families TEXT NOT NULL DEFAULT '', is_checkpoint BOOLEAN NOT NULL DEFAULT FALSE, valid_for_seconds INTEGER NOT NULL, created_utc TIMESTAMPTZ NOT NULL);",
                    @"CREATE INDEX IF NOT EXISTS idx_lane_state_transitions_created ON lane_state_transitions(created_utc);"
                ),
                new SchemaMigration(99, "Persist configuration record ownership",
