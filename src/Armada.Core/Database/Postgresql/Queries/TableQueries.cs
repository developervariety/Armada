    ...
    public static class TableQueries
    {
        ...
        public static List<SchemaMigration> GetMigrations()
        {
            return new List<SchemaMigration>
            {
                ...
                new SchemaMigration(37, "Add auto-recovery columns to merge_entries and missions",
                    @"ALTER TABLE merge_entries ADD COLUMN IF NOT EXISTS merge_failure_class INTEGER;",
                    @"ALTER TABLE merge_entries ADD COLUMN IF NOT EXISTS conflicted_files TEXT;",
                    @"ALTER TABLE merge_entries ADD COLUMN IF NOT EXISTS merge_failure_summary VARCHAR(512);",
                    @"ALTER TABLE missions ADD COLUMN IF NOT EXISTS recovery_attempts INTEGER NOT NULL DEFAULT 0;",
                    @"ALTER TABLE missions ADD COLUMN IF NOT EXISTS last_recovery_action_utc TIMESTAMPTZ;"
                )
            };
        }
