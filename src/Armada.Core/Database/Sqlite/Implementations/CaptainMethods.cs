...
#pragma warning restore CS0414
        ...
        public async Task<Captain> CreateAsync(Captain captain, CancellationToken token = default)
        {
            ...
            using (SqliteConnection conn = new SqliteConnection(_Driver.ConnectionString))
            {
                ...
                {
                    cmd.CommandText = @"INSERT INTO captains (id, tenant_id, user_id, name, runtime, model, system_instructions, allowed_personas, preferred_persona, runtime_options_json, api_endpoint_url, state, current_mission_id, current_dock_id, process_id, recovery_attempts, last_heartbeat_utc, created_utc, last_update_utc, last_process_alive_utc)
                            VALUES (@id, @tenant_id, @user_id, @name, @runtime, @model, @system_instructions, @allowed_personas, @preferred_persona, @runtime_options_json, @api_endpoint_url, @state, @current_mission_id, @current_dock_id, @process_id, @recovery_attempts, @last_heartbeat_utc, @created_utc, @last_update_utc, @last_process_alive_utc);";
                    ...
                    cmd.Parameters.AddWithValue("@api_endpoint_url", (object?)captain.ApiEndpointUrl ?? DBNull.Value);
                    ...
                    cmd.Parameters.AddWithValue("@last_process_alive_utc", captain.LastProcessAliveUtc.HasValue ? (object)SqliteDatabaseDriver.ToIso8601(captain.LastProcessAliveUtc.Value) : DBNull.Value);
