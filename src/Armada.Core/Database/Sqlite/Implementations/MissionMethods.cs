        ...
        public async Task<Mission> CreateAsync(Mission mission, CancellationToken token = default)
        {
            ...
            using (SqliteConnection conn = new SqliteConnection(_Driver.ConnectionString))
            {
                ...
                using (SqliteCommand cmd = conn.CreateCommand())
                {
                    ...
                    cmd.Parameters.AddWithValue("@reconciled_utc", mission.ReconciledUtc.HasValue ? (object)SqliteDatabaseDriver.ToIso8601(mission.ReconciledUtc.Value) : DBNull.Value);
