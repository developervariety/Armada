...
#pragma warning restore CS0414
        ...
        public async Task<Mission> CreateAsync(Mission mission, CancellationToken token = default)
        {
            ...
            using (SqliteConnection conn = new SqliteConnection(_Driver.ConnectionString))
            {
                ...
                {
                    cmd.CommandText = @"INSERT INTO missions (id, tenant_id, user_id, voyage_id, vessel_id, captain_id, title, description, status, priority, parent_mission_id, branch_name, dock_id, process_id, pr_url, commit_hash, diff_snapshot, persona, depends_on_mission_id, created_utc, started_utc, completed_utc, last_update_utc)
                            VALUES (@id, @tenant_id, @user_id, @voyage_id, @vessel_id, @captain_id, @title, @description, @status, @priority, @parent_mission_id, @branch_name, @dock_id, @process_id, @pr_url, @commit_hash, @diff_snapshot, @persona, @depends_on_mission_id, @created_utc, @started_utc, @completed_utc, @last_update_utc);";
