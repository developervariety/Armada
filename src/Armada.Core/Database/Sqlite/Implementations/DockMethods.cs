namespace Armada.Core.Database.Sqlite.Implementations
{
    ...
    /// <summary>
    /// SQLite implementation of dock database operations.
    /// </summary>
...
#pragma warning restore CS0414
        ...
        public DockMethods(SqliteDatabaseDriver driver, DatabaseSettings settings, LoggingModule logging)
        {
        ...
        public async Task<Dock> CreateAsync(Dock dock, CancellationToken token = default)
        {
            ...
            async Task CreateAsync(SqliteConnection conn, SqliteTransaction? tx)
            {
                using (SqliteCommand cmd = conn.CreateCommand())
                {
                    ...
                    DockColumns.Write(SqliteDatabaseDriver.StoredBinder.For(cmd, "docks"), dock);
                ...
                using (SqliteCommand cmd = conn.CreateCommand())
                {
                    ...
                    using (SqliteDataReader reader = await cmd.ExecuteReaderAsync(token).ConfigureAwait(false))
                    {
                        if (await reader.ReadAsync(token).ConfigureAwait(false))
                            return DockColumns.Read(reader, SqliteDatabaseDriver.StoredValues);
                    }
                }
            }
        ...
        public async Task<Dock> UpdateAsync(Dock dock, CancellationToken token = default)
        {
            ...
            using (SqliteConnection conn = new SqliteConnection(_Driver.ConnectionString))
            {
                ...
                using (SqliteCommand cmd = conn.CreateCommand())
                {
                    cmd.CommandText = @"UPDATE docks SET " + DockGitAnchorUpdate.PreserveOnSameOwnerSql(DatabaseTypeEnum.Sqlite) + @",
                            tenant_id = @tenant_id,
                            user_id = @user_id,
                            vessel_id = @vessel_id,
                ...
                using (SqliteCommand cmd = conn.CreateCommand())
                {
                    ...
                    using (SqliteDataReader reader = await cmd.ExecuteReaderAsync(token).ConfigureAwait(false))
                    {
                        while (await reader.ReadAsync(token).ConfigureAwait(false))
                            results.Add(DockColumns.Read(reader, SqliteDatabaseDriver.StoredValues));
                    }
                }
            }
        ...
        public async Task<EnumerationResult<Dock>> EnumerateAsync(EnumerationQuery query, CancellationToken token = default)
        {
            ...
            using (SqliteConnection conn = new SqliteConnection(_Driver.ConnectionString))
            {
                ...
                if (query.CreatedAfter.HasValue)
                {
                    ...
                    parameters.Add(SqliteDatabaseDriver.StoredBinder.Timestamp(new SqliteParameter(), "@created_after", "docks", "created_utc", query.CreatedAfter.Value));
                }
                if (query.CreatedBefore.HasValue)
                {
                    conditions.Add("created_utc < @created_before");
                    parameters.Add(SqliteDatabaseDriver.StoredBinder.Timestamp(new SqliteParameter(), "@created_before", "docks", "created_utc", query.CreatedBefore.Value));
                }
