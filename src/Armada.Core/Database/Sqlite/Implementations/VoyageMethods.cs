        ...
        public async Task<Voyage> CreateAsync(Voyage voyage, CancellationToken token = default)
        {
            ...
            using (SqliteConnection conn = new SqliteConnection(_Driver.ConnectionString))
            {
                ...
                using (SqliteCommand cmd = conn.CreateCommand())
                {
                    ...
                    cmd.Parameters.AddWithValue("@created_utc", SqliteDatabaseDriver.ToIso8601(voyage.CreatedUtc));
                    cmd.Parameters.AddWithValue("@completed_utc", voyage.CompletedUtc.HasValue ? (object)SqliteDatabaseDriver.ToIso8601(voyage.CompletedUtc.Value) : DBNull.Value);
                    cmd.Parameters.AddWithValue("@last_update_utc", SqliteDatabaseDriver.ToIso8601(voyage.LastUpdateUtc));
