namespace Armada.Core.Database.Sqlite.Implementations
{
    ...
    /// <summary>
    /// SQLite implementation of vessel database operations.
    /// </summary>
...
#pragma warning restore CS0414
        ...
        public VesselMethods(SqliteDatabaseDriver driver, DatabaseSettings settings, LoggingModule logging)
        {
        ...
        public async Task<Vessel> CreateAsync(Vessel vessel, CancellationToken token = default)
        {
            ...
            using (SqliteConnection conn = new SqliteConnection(_Driver.ConnectionString))
            {
                ...
                using (SqliteCommand cmd = conn.CreateCommand())
                {
                    ...
                    VesselColumns.Write(SqliteDatabaseDriver.StoredBinder.For(cmd, "vessels"), vessel);
        ...
        public async Task<EnumerationResult<Vessel>> EnumerateAsync(EnumerationQuery query, CancellationToken token = default)
        {
            ...
            using (SqliteConnection conn = new SqliteConnection(_Driver.ConnectionString))
            {
                ...
                if (query.CreatedAfter.HasValue)
                {
                    ...
                    parameters.Add(SqliteDatabaseDriver.StoredBinder.Timestamp(new SqliteParameter(), "@created_after", "vessels", "created_utc", query.CreatedAfter.Value));
                }
                if (query.CreatedBefore.HasValue)
                {
                    conditions.Add("created_utc < @created_before");
                    parameters.Add(SqliteDatabaseDriver.StoredBinder.Timestamp(new SqliteParameter(), "@created_before", "vessels", "created_utc", query.CreatedBefore.Value));
                }
