...
LENGTH(COALESCE(agent_output, '')) AS agent_output_length";
                ...
                using (SqliteCommand cmd = conn.CreateCommand())
                {
                    ...
                    using (SqliteDataReader reader = await cmd.ExecuteReaderAsync(token).ConfigureAwait(false))
                    {
                        if (await reader.ReadAsync(token).ConfigureAwait(false))
                            return MissionSummaryColumns.Read(reader, SqliteDatabaseDriver.StoredValues);
                    }
                }
            }
                    ...
                    using (SqliteCommand selectCmd = conn.CreateCommand())
                    {
                        ...
                        using (SqliteDataReader reader = await selectCmd.ExecuteReaderAsync(token).ConfigureAwait(false))
                        {
                            while (await reader.ReadAsync(token).ConfigureAwait(false))
                                results.Add(MissionSummaryColumns.Read(reader, SqliteDatabaseDriver.StoredValues));
                        }
            ...
            CancellationToken token)
        {
            ...
            using (SqliteConnection conn = new SqliteConnection(_Driver.ConnectionString))
            {
                ...
                using (SqliteCommand cmd = conn.CreateCommand())
                {
                    ...
                    AddParameters(cmd, baseParameters);
                    SqliteDatabaseDriver.StoredBinder.For(cmd, "missions").Utc("@from_utc", "created_utc", query.FromUtc);
                    SqliteDatabaseDriver.StoredBinder.For(cmd, "missions").Utc("@to_utc", "created_utc", query.ToUtc);
        ...
        private static void ApplyEnumerationFilters(EnumerationQuery query, List<string> conditions, SqliteCommand cmd)
        {
            if (query.CreatedAfter.HasValue)
            {
                ...
                SqliteDatabaseDriver.StoredBinder.For(cmd, "missions").Utc("@created_after", "created_utc", query.CreatedAfter.Value);
            }
            if (query.CreatedBefore.HasValue)
            {
                conditions.Add("created_utc < @created_before");
                SqliteDatabaseDriver.StoredBinder.For(cmd, "missions").Utc("@created_before", "created_utc", query.CreatedBefore.Value);
            }
