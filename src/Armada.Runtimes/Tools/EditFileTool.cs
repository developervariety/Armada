namespace Armada.Runtimes.Tools
{
    ...
    public class EditFileTool : IToolExecutor
    {
        ...
        /// <param name="argumentsJson">The parsed JSON arguments containing file_path, old_string, and new_string.</param>
        ...
        public async Task<ToolResult> ExecuteAsync(string toolCallId, string argumentsJson, string workingDirectory, CancellationToken cancellationToken)
        {
            ...
            try
            {
                EditFileArguments request = ToolArgumentParser.Parse<EditFileArguments>(argumentsJson);
                string filePath = request.FilePath ?? throw new ArgumentException("Required parameter 'file_path' is missing or not a string.");
                string oldString = request.OldString ?? throw new ArgumentException("Required parameter 'old_string' is missing or not a string.");
                string newString = request.NewString ?? throw new ArgumentException("Required parameter 'new_string' is missing or not a string.");
        ...
        private sealed class EditFileArguments
        {
