namespace Armada.Runtimes.Tools
{
    ...
    public class ManageDirectoryTool : IToolExecutor
    {
        ...
        /// <param name="argumentsJson">The parsed JSON arguments containing action, path, and optionally new_path.</param>
        ...
        public Task<ToolResult> ExecuteAsync(string toolCallId, string argumentsJson, string workingDirectory, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ManageDirectoryArguments request = ToolArgumentParser.Parse<ManageDirectoryArguments>(argumentsJson);
                string action = (request.Action ?? string.Empty).ToLowerInvariant();
                ...
                {
                    case "create":
                        return Task.FromResult(CreateDirectory(toolCallId, resolvedPath, timeoutSource.Token));

                    case "delete":
                        return Task.FromResult(DeleteDirectory(toolCallId, resolvedPath, timeoutSource.Token));

                    case "rename":
                        string newPath = request.NewPath ?? string.Empty;
                        ...
                        return Task.FromResult(RenameDirectory(toolCallId, resolvedPath, resolvedNewPath, timeoutSource.Token));
        ...
        private ToolResult CreateDirectory(string toolCallId, string path, CancellationToken token)
        {
        ...
        private ToolResult DeleteDirectory(string toolCallId, string path, CancellationToken token)
        {
            token.ThrowIfCancellationRequested();
            if (!Directory.Exists(path))
            {
            ...
            try
            {
                foreach (string _ in WorkspacePathPolicy.EnumerateEntries(path, token, ToolSafetyLimits.MaxEnumeratedEntries)) { }
        ...
        private ToolResult RenameDirectory(string toolCallId, string fromPath, string toPath, CancellationToken token)
        {
            token.ThrowIfCancellationRequested();
