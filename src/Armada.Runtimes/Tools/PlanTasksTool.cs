namespace Armada.Runtimes.Tools
{
    ...
    public class PlanTasksTool : IToolExecutor
    {
        ...
        /// <param name="argumentsJson">The parsed JSON arguments containing the tasks array.</param>
        ...
        public Task<ToolResult> ExecuteAsync(string toolCallId, string argumentsJson, string workingDirectory, CancellationToken cancellationToken)
        {
            ...
            PlanTasksArguments request = ToolArgumentParser.Parse<PlanTasksArguments>(argumentsJson);
            List<AgentTask> tasks = new List<AgentTask>();
            if (request.Tasks != null)
            {
                foreach (PlanTaskRequest taskRequest in request.Tasks)
                {
                    AgentTask task = new AgentTask
                    {
                        Id = taskRequest.Id ?? string.Empty,
                        Title = taskRequest.Title ?? string.Empty
                    };

                    if (taskRequest.DependsOn != null)
                    {
                        foreach (string dependency in taskRequest.DependsOn)
                        {
                            task.DependsOn.Add(dependency ?? string.Empty);
                        }
                    }
        ...
        private sealed class PlanTasksArguments
        {
            public List<PlanTaskRequest>? Tasks { get; set; }
        }

        private sealed class PlanTaskRequest
        {
