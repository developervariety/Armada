...
/**
 * Cached tokens for one model. This fork's per-model breakdown reports cache reads and cache
 * writes separately, where bucket-level totals carry a single combined figure. Reading a
 * `cachedTokens` field off a model breakdown yields undefined and renders as a blank cell.
 */
function modelCachedTokens(model: TokenUsageModelBreakdown): number {
            ...
            <svg ref={modelChartRef} width="100%" viewBox={`0 0 800 ${modelChartHeight}`} preserveAspectRatio="xMidYMid meet" style={{ display: 'block' }}>
              ...
              {byModel.map((m, i) => {
                ...
                const segs = metric === 'byType'
                  ? [{ v: m.inputTokens, c: TYPE_COLORS.input }, { v: m.outputTokens, c: TYPE_COLORS.output }, { v: modelCachedTokens(m), c: TYPE_COLORS.cached }]
