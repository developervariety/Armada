...
vi.mock('../api/client', () => ({
  ...
  previewPersonaPrompt: vi.fn(),
  updateProjectProfile: vi.fn(),
}));

import { listAllFleets, listAllVessels, listFleets, listVessels } from '../api/client';
import ProjectProfileDetail from './ProjectProfileDetail';
...
test('offers every fleet and vessel in the scope pickers, not only the first server page', async () => {
  render(
    <MemoryRouter initialEntries={['/project-profiles/new']}>
      <Routes><Route path="/project-profiles/:id" element={<ProjectProfileDetail />} /></Routes>
    </MemoryRouter>,
  );
