...
import { canEditOwned, resolveCreateScope, viewerFromAuth, OWNED_RECORD_WRITE_LEVEL } from '../lib/scoping';
...
/** Ownership fields of an endpoint in the shape the shared rules read. */
function owned(endpoint: ModelEndpoint) {
  return { ownershipScope: endpoint.scope, tenantId: endpoint.tenantId, userId: endpoint.userId };
}
...
export default function Endpoints() {
  ...
  // The server runs a health sweep for a global administrator only.
  const canSweep = auth.isAdmin;
  const canManage = auth.isAdmin || auth.isTenantAdmin;
  const writeLevel = OWNED_RECORD_WRITE_LEVEL.modelEndpoints;
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      ...
      if (editing) {
        // Scope is fixed at creation, so an update carries the other fields only.
        const updated = await updateModelEndpoint(editing.id, payload);
        setShowForm(false);
        pushToast('success', t('Endpoint "{{name}}" saved.', { name: updated.name }));
      } else {
        const created = await createModelEndpoint({ ...payload, scope: form.scope });
        setShowForm(false);
  ...
  const filtered = useMemo(() => endpoints.filter((endpoint) => {
    ...
    const matchesSearch = needle.length === 0
      ...
      || (endpoint.model || '').toLowerCase().includes(needle)
  ...
  async function handleSweep() {
    ...
    try {
      ...
      pushToast('success', t('Health sweep probed {{count}} endpoint(s).', { count: result.endpointsProbed }));
      await load();
    } catch (err: unknown) {
  ...
  return (
    <div>
      <PageHeader
        title={t('Endpoints')}
        subtitle={t('Managed embedding and inference model endpoints. A health sweep probes every enabled endpoint; Validate sends a real request to the configured model.')}
        actions={(
          <>
            <AutoRefreshSelect seconds={refreshSeconds} onChange={setRefreshSeconds} />
            <RefreshButton onRefresh={load} title={t('Refresh endpoints')} />
            {canSweep && (
              <button className="btn" onClick={handleSweep} disabled={sweeping} title={t('Probe every enabled endpoint.')}>
      ...
      {showForm && (
        <div className="modal-overlay" onClick={() => setShowForm(false)}>
          <form className="modal modal-large" onClick={(e) => e.stopPropagation()} onSubmit={handleSave}>
            ...
            <ScopeSelect viewer={viewer} value={form.scope} locked={!!editing} onChange={(scope) => setForm({ ...form, scope })} />
      ...
      {health.open && health.endpoint && (
        <div className="modal-overlay" onClick={() => setHealth({ open: false, endpoint: null })}>
          <div className="modal modal-large" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-actions">
              {canEditOwned(viewer, owned(health.endpoint), writeLevel) && (
      ...
      {loading && endpoints.length === 0 ? (
        ...
        <div className="playbook-empty-state">
          ...
          <span>{canManage ? t('Add an embedding or inference endpoint to manage and monitor it.') : t('Add a personal endpoint, or ask a tenant administrator to configure shared model endpoints.')}</span>
        </div>
      ) : (
          ...
          <table>
            ...
            <tbody>
              {filtered.map((endpoint) => {
                const canEditRow = canEditOwned(viewer, owned(endpoint), writeLevel);
                return (
