    ...
    public class MergeQueueRoutes
    {
            ...
            IAuthorizationService authz)
        {
            ...
            app.Post<PurgeMergeEntriesRequest>("/api/v1/merge-queue/purge", async (ApiRequest req) =>
            {
                ...
                // A tenant administrator purges only entries its own tenant owns; an entry of another
                // tenant is reported as not found, exactly as the single-entry purge reports it.
                if (!ctx.IsAdmin && String.IsNullOrEmpty(ctx.TenantId))
                    return RouteAuthRefusal.Refuse(req, ctx);
            ...
            api => api
                ...
                .WithDescription("Permanently deletes multiple terminal merge queue entries from the database by ID. A tenant administrator purges only its own tenant's entries; any other ID is reported as not found. Returns a summary of purged and skipped entries. This cannot be undone.")
