namespace Armada.Server.Mcp.Tools
{
    ...
    public static class McpInboxTools
    {
        ...
        public static void Register(RegisterToolDelegate register, DatabaseDriver database, LoggingModule logging)
        {
            ...
            register(
                "inbox",
                "Return the operator's inbox: everything across the fleet that requires a human's attention or action right now, ordered most-urgent first. Use this to answer questions like \"Is there anything waiting on me?\", \"Is there anything that needs my attention?\", or \"Do I have any action items from Armada work?\". It surfaces two kinds of item: (a) work awaiting your decision (human-in-the-loop) -- missions in Review to approve or reject, and deployments pending approval; and (b) autonomous work that failed and needs intervention (human-out-of-the-loop) -- failed missions and missions whose work could not land (landing failed) while their voyage is still live, open or monitoring incidents (open_incident; a failed mission whose voyage has halted is carried by its incident, not listed twice), failed merges, failed or verification-failed deployments, and stalled captains. Purely informational events such as completions or normal progress are deliberately excluded. Each item has: kind, severity (Critical, Warning, or Info), title, detail, the related entity (entityType and entityId), and a dashboard href. An empty items list means nothing currently needs the operator's attention.",
