namespace Armada.Test.Unit.Suites.Services
{
    ...
    public class PerStagePreferredModelTests : TestSuite
    {
        ...
        protected override async Task RunTestsAsync()
        {
            ...
            await RunTest("Dispatch_StagePreferredModel_ClearsIncompatibleDispatchCaptainPin", async () =>
            {
                using (TestDatabase testDb = await TestDatabaseHelper.CreateDatabaseAsync())
                {
                    LoggingModule logging = CreateLogging();
                    ...
                    Vessel vessel = await CreateVesselAsync(testDb, "pin-clear-vessel").ConfigureAwait(false);

                    Captain sonnetPin = new Captain("sonnet-pin-captain");
                    sonnetPin.Model = "claude-sonnet-4-6";
                    sonnetPin.AllowedPersonas = "[\"Worker\",\"Judge\"]";
                    sonnetPin.State = CaptainStateEnum.Idle;
                    sonnetPin = await testDb.Driver.Captains.CreateAsync(sonnetPin).ConfigureAwait(false);

                    Pipeline pipeline = new Pipeline("ReviewedPinClear");
                    pipeline.Stages = new List<PipelineStage>
                    {
                        new PipelineStage(1, "Worker"),
                        new PipelineStage(2, "Judge") { PreferredModel = "claude-opus-4-7" }
                    ...
                    List<MissionDescription> missions = new List<MissionDescription>
                    {
                        new MissionDescription("Pin clear work", "Worker keeps pin; Judge drops pin for opus pool")
                        {
                            PreferredCaptainId = sonnetPin.Id,
                            PreferredModel = "claude-sonnet-4-6"
                        }
                    };
                    ...
                    Voyage voyage = await admiral.DispatchVoyageAsync(
                        "Pin clear voyage",
                        "Stage model clears incompatible captain pin",
                        vessel.Id,
                        missions,
                        pipeline.Id).ConfigureAwait(false);
                    ...
                    AssertEqual(sonnetPin.Id, workerMission!.PreferredCaptainId,
                        "Worker should keep dispatch captain pin when stage has no PreferredModel");
                    AssertNull(judgeMission!.PreferredCaptainId,
                        "Judge with stage PreferredModel incompatible with pin should clear PreferredCaptainId");
                    AssertEqual("claude-opus-4-7", judgeMission.PreferredModel,
                        "Judge should still target stage opus model");
                }
            });

            await RunTest("Dispatch_StagePreferredModel_KeepsDispatchCaptainPinWhenCompatible", async () =>
            {
                using (TestDatabase testDb = await TestDatabaseHelper.CreateDatabaseAsync())
                {
                    LoggingModule logging = CreateLogging();
                    ...
                    Vessel vessel = await CreateVesselAsync(testDb, "pin-keep-vessel").ConfigureAwait(false);

                    Captain dualPin = new Captain("dual-opus-captain");
                    dualPin.Model = "claude-opus-4-7";
                    dualPin.AllowedPersonas = "[\"Worker\",\"Judge\"]";
                    dualPin.State = CaptainStateEnum.Idle;
                    dualPin = await testDb.Driver.Captains.CreateAsync(dualPin).ConfigureAwait(false);

                    Pipeline pipeline = new Pipeline("ReviewedPinKeep");
                    pipeline.Stages = new List<PipelineStage>
                    {
                        new PipelineStage(1, "Worker"),
                        new PipelineStage(2, "Judge") { PreferredModel = "claude-opus-4-7" }
                    ...
                    List<MissionDescription> missions = new List<MissionDescription>
                    {
                        new MissionDescription("Pin keep work", "Both stages can use opus pin")
                        {
                            PreferredCaptainId = dualPin.Id,
                            PreferredModel = "claude-sonnet-4-6"
                        }
                    };
                    ...
                    Voyage voyage = await admiral.DispatchVoyageAsync(
                        "Pin keep voyage",
                        "Compatible pin retained on stage with PreferredModel",
                        vessel.Id,
                        missions,
                        pipeline.Id).ConfigureAwait(false);
                    ...
                    foreach (Mission m in voyageMissions)
                    {
                        AssertEqual(dualPin.Id, m.PreferredCaptainId,
                            "Pin should be kept when captain matches stage persona and stage PreferredModel (" + m.Persona + ")");
                    }
                }
            });
        }
