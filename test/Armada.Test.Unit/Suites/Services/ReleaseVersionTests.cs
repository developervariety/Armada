namespace Armada.Test.Unit.Suites.Services
{
    ...
    {
        private static readonly string[] _StaleReleaseVersions = { "0.5.0", "0.4.0", "0.3.0" };

        public override string Name => "Release Version";

        protected override async Task RunTestsAsync()
        {
            await RunTest("ProductVersion And Shared Build Props Match V060", () =>
            {
                ...
                AssertEqual("0.6.0", Constants.ProductVersion);
            ...
            await RunTest("Helm Program Uses ProductVersion Constant", () =>
            {
                ...
                AssertFalse(programContents.Contains("\"0.6.0\""), "Helm entry point should not contain a hard-coded release version literal");
                AssertFalse(programContents.Contains("\"v0.6.0\""), "Helm entry point should compose the prefixed version instead of hard-coding it");
                AssertFalse(programContents.Contains("SetApplicationVersion(\"0.6.0\")"), "Helm CLI version should not be hard-coded");
            });
