        ...
        protected override async Task RunTestsAsync()
        {
            ...
            await RunTest("ReserveEngages_ConstrainedHighTierWithInFlightDemand_DefersWorker", async () =>
            {
                using (TestDatabase testDb = await TestDatabaseHelper.CreateDatabaseAsync())
                {
                    ...
                    await CreateIdleCaptainAsync(testDb, "reserve-opus-a", "claude-opus-5");
                    await CreateIdleCaptainAsync(testDb, "reserve-opus-b", "claude-opus-5");
            ...
            await RunTest("ZeroReservation_ConstrainedHighTier_WorkerDispatches", async () =>
            {
                using (TestDatabase testDb = await TestDatabaseHelper.CreateDatabaseAsync())
                {
                    ...
                    await CreateIdleCaptainAsync(testDb, "zero-opus-a", "claude-opus-5");
                    await CreateIdleCaptainAsync(testDb, "zero-opus-b", "claude-opus-5");
