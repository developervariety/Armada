namespace Armada.Test.Unit.Suites.Database
{
    ...
    public class StoredScopeSqlTests : TestSuite
    {
        ...
        protected override async Task RunTestsAsync()
        {
            ...
            await RunTest("ProjectProfile scope conditions keep their text, order and parameters on every provider", () =>
            {
                string postgresql = "tenant_id = @tenant_id AND user_id = @user_id AND scope = @scope AND fleet_id = @fleet_id AND vessel_id = @vessel_id AND (name ILIKE @search OR COALESCE(description, '') ILIKE @search) AND active = @active AND created_utc::timestamptz >= @from_utc AND created_utc::timestamptz <= @to_utc";
                string others = "tenant_id = @tenant_id AND user_id = @user_id AND scope = @scope AND fleet_id = @fleet_id AND vessel_id = @vessel_id AND (LOWER(name) LIKE @search OR LOWER(COALESCE(description, '')) LIKE @search) AND active = @active AND created_utc >= @from_utc AND created_utc <= @to_utc";
                ProjectProfileQuery everyFilter = new ProjectProfileQuery
                {
                    TenantId = "ten_x", UserId = "usr_x", Scope = ProjectProfileScopeEnum.Vessel, FleetId = "flt_x", VesselId = "vsl_x", Search = "Find", Active = true,
                    FromUtc = new DateTime(2026, 1, 1, 0, 0, 0, DateTimeKind.Utc), ToUtc = new DateTime(2026, 1, 2, 0, 0, 0, DateTimeKind.Utc)
                };
                AssertScope(p => p == DatabaseTypeEnum.Postgresql ? postgresql : others,
                    p => ProjectProfileMethods.Scope(ProjectProfileMethods.Table.Filter(), everyFilter, p),
                    p => ProjectProfileMethods.Scope(ProjectProfileMethods.Table.Filter(), new ProjectProfileQuery(), p));
                AssertEqual("is_default DESC, last_update_utc DESC, name ASC", ProjectProfileMethods.Order);
                AssertTableMatchesWriter(ProjectProfileMethods.Table, new ProjectProfile());
            });
