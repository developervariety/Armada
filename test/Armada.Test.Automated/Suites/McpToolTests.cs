    ...
    public class McpToolTests : TestSuite
    {
        ...
        protected override async Task RunTestsAsync()
        {
            ...
            await RunTest("AllTools_ArmadaStopAll_Executes", async () =>
            {
                ...
                JsonElement beforeResult = await CallToolAsync("armada_enumerate", new { entityType = "captains", pageSize = 1000 }).ConfigureAwait(false);
                ...
                List<Captain> working = before.Objects
                    .Where(c => c.State == Armada.Core.Enums.CaptainStateEnum.Working && !String.IsNullOrEmpty(c.CurrentMissionId))
                    .ToList();
                ...
                {
                    JsonElement missionResult = await CallToolAsync("armada_mission_status", new { missionId = captain.CurrentMissionId }).ConfigureAwait(false);
                    ...
                    bool stillRunning = mission.CaptainId == captain.Id
                        && (mission.Status == Armada.Core.Enums.MissionStatusEnum.Assigned || mission.Status == Armada.Core.Enums.MissionStatusEnum.InProgress);
