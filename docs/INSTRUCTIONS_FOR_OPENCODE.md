> **This document is meant to be provided to OpenCode as system context and paired with an MCP config that points at Armada.** It gives OpenCode everything it needs to orchestrate and interact with Armada. Copy the contents below into your OpenCode system prompt or an `AGENTS.md`-style context file.

---

## Connecting OpenCode to Armada over MCP

OpenCode talks to Armada through Armada's MCP server. The Admiral serves the modern **MCP Streamable HTTP** transport at `http://localhost:7891/mcp` (MCP port 7891; REST is on 7890). This is the recommended way to connect. OpenCode keeps its MCP servers in a config file (`opencode.json` / `opencode.jsonc`) under an `mcp` block, and the `opencode mcp` command group manages them.

### Option A -- CLI (`opencode mcp add`)

Add the Armada server as a remote HTTP server in one command:

```bash
opencode mcp add armada --url http://localhost:7891/mcp
```

This writes an `mcp` entry into your OpenCode config (`~/.config/opencode/opencode.json` on Linux/macOS, or the equivalent user config directory on Windows). List the configured servers and their live status:

```bash
opencode mcp list
```

Once the Admiral is running, the `armada` row shows an online glyph with its discovered tool count. If the Admiral is not up yet you will see a connection error instead -- start it with `armada server start` and re-run `opencode mcp list`.

### Option B -- Config file (headless / scripted runs)

You can edit the OpenCode config directly instead of using the CLI. Add an `mcp` block with a remote server that points at the Admiral's MCP path:
...
OpenCode loads this config on startup, so the Armada tools are available to every interactive session and every headless `opencode run`:

```bash
opencode run --format json --auto --dir . "what is the status of the fleet?"
```

### Option C -- stdio bridge (fallback / when an auth header is required)

If your deployment fronts the MCP port with a proxy that requires an auth header, use Armada's **stdio bridge** (`armada mcp stdio`), which reuses the local `armada` CLI's credentials. Configure it as a local server so OpenCode launches Armada as a subprocess:
...
### Registering an OpenCode captain in Armada

Armada drives OpenCode headlessly, so an OpenCode captain is just a captain whose runtime is `OpenCode`:

```bash
armada captain add opencode-1 --runtime opencode --model openai/gpt-4o
```

The `--model` value is an OpenCode `provider/model` identifier (for example `openai/gpt-4o` or `anthropic/claude-3-5-sonnet`), the same form `opencode run --model` expects. Authenticate the provider with `opencode providers login` before dispatching so the runtime can reach it. A captain's reasoning effort is translated to OpenCode's `--variant` flag on each run: Minimal and Low map to `minimal`, Medium and High map to `high`, and Off leaves the flag off so OpenCode keeps its provider default. Requesting thinking output adds `--thinking`, and Armada always launches with `--auto` so permissions that are not explicitly denied are approved without prompting.

### Notes

`armada mcp install` configures Claude Code, Codex, Gemini, and Cursor, but it does not configure OpenCode. Wire OpenCode up yourself with `opencode mcp add` or by editing the config as shown above -- both write to the same `mcp` block.

Verify the connection with `opencode mcp list` -- the `armada` server should appear online with a non-zero tool count. Once connected, the Ask Armada page stops showing the "not connected to Armada over MCP" banner for this captain.
...
enumerate({ entityType: "vessels" })             -> find vessels
```
...
- You run headless via `opencode run --auto` by default -- all permissions that are not explicitly denied are auto-approved. Avoid destructive operations outside your worktree.
