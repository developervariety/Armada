...
Core services: 217 fork files and 132 upstream files; 38 upstream-only, 123 fork-only, 52 different shared files, and 42 equal shared files. Server: 189 fork files and 156 upstream files; 18 upstream-only, 51 fork-only, 92 different shared files, and 46 equal shared files.
...
| Ask Armada and planning chat enrichment | Adapt | U `Server/CaptainChatService.cs:82` scopes captain reads; `:160` separates API tool cards; `:337` handles OpenCode JSONL and thinking; `:423` mints a caller token for API-runtime MCP access. U `PlanningSessionCoordinator.cs:214` adds stream/thinking controls. F already has captain chat, turn metrics and cancellation. | Keep fork coordination, timeouts, resource gates, and dispatch service. Port useful presentation and parser behavior without changing operator tool authority. Test actual runtime fixtures, stop then next turn, errors, no raw protocol text in reply, per-turn tool IDs, scoped event subscribers, and token expiry. API runtime dependency comes later. Implemented since: the fork runtime adapters already reduce OpenCode JSONL to answer text and redacted `[ARMADA:ACTIVITY]` records, but chat and planning appended those records to the answer. Captured OpenCode events replayed through the real transform gave a chat reply and a stored planning message that began with `[ARMADA:ACTIVITY] tool read src/File.cs (ok)`. One public rule, `ActivityRecords`, now keeps every activity record out of chat and planning text and reads tool records back into `ask.tool` and `planning-session.tool` cards (one card per call, name, redacted primary argument, success); mission output uses the same rule. Not imported: upstream's root `text` and `message.content` extraction (the fork deliberately ignores `message.content`), full tool arguments and output in cards (fork activity records redact them and omit output), and the API and remote runtimes. Stop then next turn, scoped subscribers and token expiry were not re-tested here. |
...
| Telemetry log export | Adapt | Both tips already have `ArmadaTelemetryHost`, `ArmadaMetrics`, token usage entities/capture, and summary routes. U `Server/ArmadaTelemetryHost.cs:93-183` enables Loki/OTLP logs and bridges SyslogLogging, while F exports metrics/traces only. | Preserve disabled-by-default behavior and redaction. U forwards raw message/exception values; apply the fork data boundary before export. Add a counted forwarding failure path instead of the upstream empty catch. Verify one entry reaches an isolated sink, no recursion, no double subscription after restart/dispose, and errors retain stack traces. Implemented since: the fork host enabled the Loki exporter but never bridged the Admiral log stream, so a configured Loki endpoint received no Admiral logs. `ArmadaTelemetryHost` now enables log export only when a Loki or OTLP endpoint is set, at Information and above, and forwards `LoggingModule.MessageLogged` through `TelemetryLogExport`: Debug is not exported, message and exception text (with stack) pass through `SecretRedactor` and are bounded to 16,000 characters, and the exception object is never exported. Forwarding failures are counted instead of an empty catch, a re-entry guard prevents recursion, and dispose unsubscribes so a restart exports each entry once. Telemetry stays disabled by default. Not verified: delivery to a live Loki or OTLP collector (tests use a local unused port). Not adopted: upstream's Docker observability stack (telemetry on by default, Grafana admin/admin), its aspnet-only server Dockerfile, script rewrites that drop fork safeguards, and the SyslogLogging 2.2.2 and Watson 7.1.1 bumps (no concrete reason; 2.2.1 already has the log event the bridge uses). |
...
| Server rebuild slots | Reject for the container target; retain hardened fork SelfDeploy for process-owned hosts | U `ServerRebuildService.cs:175-204,256,333-345` adds slots and Harbor health rollback. It uses `McpToolHelpers.PerformBackupAsync/PerformRestoreAsync`, which are SQLite-specific. U `:517-520` continues after backup failure. F `Services/SelfDeployService.cs`, the build runner and `SelfDeployCutoverCoordinator` support the fork path, with verified process identity, immutable artifacts, bounded health rollback and restart recovery; in-container self-deploy fails closed. The value decision is recorded in `implementation-followups.md` FOLLOWUP-018. | Do not enable a competing deploy mechanism for the current service. Require provider-specific backup/restore and tested rollback before use; keep dispatch hold/drain semantics, deployed commit verification, data migration rollback rules, and fork process ownership. Rollback after new writes needs an explicit data-loss policy. |
...
| Mission lifecycle and interrupted recovery | Retain and reconcile narrow fixes | Both have central `MissionStateMachine`. U `AdmiralService.cs:1666` requeues interrupted exits. F has deeper quota/safeguard rerouting, process identity and completion handling. | Prove interruption and duplicate exit behavior using fork lifecycle tests. Keep handoff boundaries, retry budgets, actual process identity and output-after-exit handling. Do not import U `McpToolHelpers.IsValidTransition`, which duplicates and omits state-machine cases. |
...
| Native memory and Recorder | Defer; do not activate | U `Services/MemoryService.cs`, memory routes/tools and Recorder persona add an independent store. U `PromptTemplateService.cs:166-245` automatically appends recall guidance to built-in working templates on startup. Owner memory policy requires one durable source. | Keep the current memory source. Do not seed Recorder, append recall sections, or start an independent memory store as an incidental port. The accepted design objective now evaluates Git-authoritative capture and indexed retrieval with exact provenance, reviewed Recorder proposals, and measured context loading. This is an improvement opportunity, not a permanent feature rejection. |
| Operational control, coordination and autonomy | Retain shared fork additions | F-only `CoordinationService`, `AutonomousObjectiveScheduler`, `AgentWakeProcessHost`, dispatch holds, inbox stale-peer support, operational asset audit, production verification and unlanded-branch tools have no U equivalent. | Preserve shared tool registration and route/event wiring. Add regressions at real entry points when a common server file changes. Keep participant headers, wakes, scheduler persistence, policy opt-outs and direct-edit authority. |
