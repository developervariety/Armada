...
| TryLogin RejectsInvalidProof | 40 | ADD | ADD: invalid proof denial with reason. No retired lead/Grok dependency. |
...
| `Services.MissionPrompt.generate_claude_md_async_includes_model_context_when_enabled_and_set` | Awaiting owner decision | None | generated instructions are written under .armada/instructions and carry no Model Context Updates section |
| `Services.MissionPrompt.generate_claude_md_async_includes_update_instructions_even_when_model_context_is_empty` | Awaiting owner decision | None | generated instructions carry no Model Context Updates section |
| `Services.MissionPrompt.template_resolved_claude_md_contains_model_context_updates_when_enabled` | Awaiting owner decision | None | generated instructions carry no Model Context Updates section |
| `Services.PersonaSeedService.seed_async_creates_new_built_in_personas_and_expanded_full_pipeline` | Awaiting owner decision | None | the fork seeds FullPipeline as Architect, Worker, TestEngineer, Judge and does not seed a Test Engineer persona |
| `Services.PersonaSeedService.seed_async_upgrades_the_legacy_built_in_full_pipeline_order` | Awaiting owner decision | None | the fork does not upgrade FullPipeline to the expanded product persona order |
| `Services.PersonaSeedService.seed_async_renames_the_legacy_built_in_test_engineer_persona` | Awaiting owner decision | None | the fork does not rename the built-in TestEngineer persona to Test Engineer |
