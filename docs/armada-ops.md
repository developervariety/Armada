...
2. Create a git worktree at `<dock-worktree>/<RelativePath>` (e.g. `../JproDeobfuscator`).
...
Git-ignored extraction outputs (large data files generated by a tool, e.g. JPRO deobfuscation
artifacts) cannot be committed into the sibling repo. They exist on the Admiral host in the
...
- Each entry in `ExtractionArtifactPaths` is a relative path under `WorkingDirectory`
  (e.g. `output/jpro-export`) that is copied recursively.
...
### Example: otrbuddy + JproDeobfuscator

```json
[
  {
    "vesselRef": "vsl_xxxx",
    "repoUrl": "https://github.com/example/JproDeobfuscator.git",
...
Set `WorkingDirectory` on the JproDeobfuscator vessel to the operator's local checkout (e.g.
`C:\dev\project\JproDeobfuscator`). After running the JPRO extraction tool locally, the next
dock provisioned for otrbuddy will copy `output/jpro-export/` into the sibling worktree so
the otrbuddy MSBuild probes and data-tests resolve correctly.
