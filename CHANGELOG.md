...
- The admiral image pins ALL agent-CLI versions (`@anthropic-ai/claude-code@2.1.263`, `@openai/codex@0.153.4`, `@google/gemini-cli@0.58.0`, `@nothumanwork/cursor-agent-cli@1.1.0`, `opencode-ai@1.18.29`) so rebuilds are reproducible; codex 0.153.4 is the CLI whose model list includes the `gpt-6-astra` frontier model.
...
- An objective whose original voyage failed but whose autonomous rescue landed reconciles to `Completed` instead of sitting `InProgress` for ever. The scheduler's completion check no longer requires every linked voyage to be `Complete`: a terminal `Failed`/`Cancelled` voyage no longer blocks completion when a linked `Complete` rescue voyage re-did its work (identified by the rescue stage carrying one of the failed voyage's missions as its `ParentMissionId`), while an unrescued failure still blocks so an objective whose work never landed is never false-completed.
