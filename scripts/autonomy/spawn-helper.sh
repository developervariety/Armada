...
run_helper() {
    ...
    local prompt_file="$2"
    local helper_cwd="$3"
    local prompt participant_key helper_name contract
...
and exit. Do not start a polling loop or recurring schedule.
EOF
)
    ...
    case "$(printf '%s' "$runtime" | tr '[:upper:]' '[:lower:]')" in
        ...
        claude) exec claude --print --setting-sources project,local --strict-mcp-config "$prompt" ;;
...
do_spawn() {
    local name="${1:-}"
    local prompt_arg="${2:-}"
    local helper_cwd_arg="${3:-$WORKDIR}"
    validate_name "$name"
    ...
    local prompt_file helper_cwd participant_key out_log pid alive
    ...
    echo "spawned name=$name pid=$pid key=$participant_key log=$out_log"
}
...
do_list() {
    local file pid name state start now elapsed key
    now=$(date -u +%s)
    printf '%-20s %-8s %-10s %-10s %s\n' NAME PID STATE ELAPSED PARTICIPANT_KEY
    for file in "$RUN_DIR"/*.pid; do
        [ -e "$file" ] || continue
        name=$(basename -- "$file" .pid)
        pid=$(cat "$file" 2>/dev/null || true)
...
case "$COMMAND" in
    ...
    list) do_list ;;
    cull) do_cull ;;
    *) fail "usage: $0 {spawn <name> <prompt-file> [cwd] | kill <name> | list | cull}" ;;
esac
