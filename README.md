...
The 2026-08-19 pass extracted upstream's project profiles, skills, per-persona captain overrides, coordination leases, and the process-liveness/output-heartbeat split. Extracting them surfaced that several had been ported in an earlier pass as data and routes with no consumer, and the entries below record where the fork's version now differs from upstream's.
