...
The 2026-08-19 pass extracted upstream's project profiles, skills, per-persona captain overrides and their dispatch UI, coordination leases, and the process-liveness/output-heartbeat split. Upstream's wider dashboard nav and layout rewrite remains unabsorbed, per the paragraph above; the captain-assignment, chat, and entity pages were taken from it, the nav and layout consolidation was not. Upstream's Touchstone test harness is likewise not absorbed: adopting it would replace the infrastructure under this fork's own suites. Extracting them surfaced that several had been ported in an earlier pass as data and routes with no consumer, and the entries below record where the fork's version now differs from upstream's.
