...
| `egressExcludedVesselIds` | empty | Vessels whose content must never leave the host. Every decision about a mission or target vessel on one of them keeps its rule, sends nothing, and records an unavailable event with reason `egress_excluded_vessel`. |
