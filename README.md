...
| `egressExcludedMarkers` | empty | Text markers meaning a state carries content that must never leave the host (for example a directory of decrypted vendor data). Checked on the UNREDACTED state by every egress path - adapters, custom decisions and captain tools. A decision's or custom definition's own `egressExcludedMarkers` replaces this list for it; an empty own list opts it out. A match sends nothing and records `egress_excluded_content`. |
...
Every short-lived child process the admiral starts (git, `gh`/`glab`, `docker`, version and `command -v` probes, check, gate and merge-queue test commands, and the `run_command` tool) runs through one bounded runner in `Armada.Core`. It reads both output streams at once, so a child that fills one pipe while the other is open never blocks. Each stream keeps a per-call byte budget; past it the result keeps the beginning and the end and names the omitted bytes. A timeout and a caller cancellation both kill the process tree (and the process group, for shell commands), and the readers get a bounded drain window after exit or kill, so a background child holding a pipe cannot hang the call. Git runs with terminal, credential-manager and pager prompts off; network git (fetch, push) uses the git timeout (`ARMADA_GIT_TIMEOUT_MS`, default 120 s). Long-lived and two-way processes (captain runtimes, OpenCode servers, MCP stdio sessions, the self-deploy host) keep their own supervision.
