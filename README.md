...
This is a private fork of [`jchristn/Armada`](https://github.com/jchristn/Armada). It tracks upstream (last sync: 2026-08-19, merge-base `e9e3021f`, selective extraction rather than a merge, so the merge-base does not move; upstream is 206 commits ahead at that point) and ships at v0.9.0, preserving upstream's v0.8.0 delivery-management model while adding several subsystems that turn Armada from a captain launcher into an autonomous, retrieval-aware delivery platform. Selected upstream v0.9.0 features (workspace terminal + diff, landing-retry conflict capture, and the needs-you inbox) have been ported back into the fork as first-class features and are listed below. The dashboard is aligned to upstream's consolidated navigation and styling (re-aligned 2026-08-24 after a partial extraction produced a broken shell); the fork keeps its own dashboard surfaces on top of it — token usage, the code-index page, the coordination chatroom, provider credential fields, notifications, dispatch captain assignment, release webhooks, and persona captain preference. The rest of upstream's v0.9.0 — its Voltaic-based MCP migration, Touchstone test harness, and OpenTelemetry stack — is deliberately not absorbed because the fork's own implementations of those surfaces are more developed.

The 2026-08-19 pass extracted upstream's project profiles, skills, per-persona captain overrides and their dispatch UI, coordination leases, and the process-liveness/output-heartbeat split. The dashboard nav and layout rewrite is absorbed, with two destinations added back that upstream has no counterpart for or folded away. Upstream's Touchstone test harness and its consolidated navigation are now both absorbed, the harness alongside this fork's own suites rather than in place of them. Extracting them surfaced that several had been ported in an earlier pass as data and routes with no consumer, and the entries below record where the fork's version now differs from upstream's.
...
- **Per-persona captain overrides, end to end.** A dispatch may bind a pipeline step to a preferred captain plus a fallback tier, applied to every mission of that persona including Architect fan-out. Upstream persists these once per dispatch surface; the fork routes REST and MCP through a single `VoyageDispatchService` and persists there, so the overrides cannot be stored by one caller and dropped by the other. The fallback tier is a `CaptainTierEnum` (`Economy` / `Standard` / `Premium`), which is a different vocabulary from `preferredModel`'s tiers (`59803083`). The Dispatch page carries a per-step picker and fallback-tier select, each step seeded from its persona's default captain, and a step left untouched sends no override so normal routing still applies. A persona's default captain is settable on its detail page and persisted for all four providers; its column had been added by a migration that no INSERT, UPDATE, or mapper ever carried (`896fd0ce`, `0e4bde49`).
- **Persona eligibility matched by name, not substring.** A captain's `AllowedPersonas` is deserialized and compared through `PersonaCatalog`, which normalizes the two persisted spellings of a persona. The previous quoted-substring test against the raw JSON treated `"Test Engineer"` and `"TestEngineer"` as different personas, so a captain carrying the legacy spelling was eligible for nothing and its missions queued for ever while it sat Idle — a spelling fault that presents as a capacity fault (`98f9ad1d`).
...
- **Host-wide command serialization.** The in-dock build/test gate holds a dock lease across a host-wide queue shared by every caller that runs a vessel's build and test commands — the mission definition-of-done gate, check runs including dispatch-armed checks, and merge-queue test runs — so they run one at a time instead of crashing the test host. A check submitted during a gate queues rather than racing, so a slow check is an expected outcome and not a symptom.
- **Checks armed on every dispatch.** Every dispatched voyage carries Build and UnitTest checks, not just rescue voyages, so a voyage cannot reach its Judge stage bare — a Judge PASS is rejected without a green independent check, so a bare voyage was already condemned when it started and nothing said so until the whole pipeline had run. Cancelling a voyage discards its checks, so a re-dispatch previously started bare again. The checks are armed Pending and become runnable when a stage has committed work to its branch: eligibility follows the work rather than voyage completion, which had deadlocked the pair — the voyage waited on the Check's green while the Check waited on the voyage completing, so no armed record ever ran (`bb906cbd`). Before executing, the record is stamped with the branch and commit it measures, and Build and UnitTest run in an isolated checkout resolved from the record, so an unstamped Check cannot report green against the default branch; cancelled or failed voyages stay ineligible. A type is armed only when the profile defines its command, and never twice for one voyage.
- **Named rejection when checks are missing or failed.** A Judge PASS rejected by the real-signal gate names the checks that blocked it and states that every failed check must be resolved, so an operator is not left enumerating a voyage's checks by hand.
...
- **Ineffective-rescue rejection.** An Implementation-mode rescue whose change set is empty or entirely documentation fails as `ineffective_rescue` with the change set named. The no-op detector cannot catch this: it requires a FAST exit, and the case here ran twenty-four hours, drew stall nudges, died on a runtime crash, and left one documentation file behind while every liveness measure called it a working rescue. Audit and Research rescues are exempt, and the rescue's paths are deliberately not compared against the original's, because a rescue is expected to rewrite the same files from scratch.
- **Verified stage handoff base.** A downstream pipeline stage proves its checkout contains the commit its predecessor produced before a captain is launched, failing as `stage_base_missing` when it demonstrably does not. Inheriting a branch name is not inheriting its commit: one Worker's dock was cut without the preceding stage's commit, rebuilt on a base still carrying errors that stage had already fixed, failed on them, and took ten downstream missions with it — while every symptom argued for reading the Worker's own diff. A base that cannot be proved is recorded as unverified rather than treated as correct.
- **Stall detection that can actually fire.** The process-liveness loop refreshes a captain's `LastProcessAliveUtc` rather than its output heartbeat, so `LastHeartbeatUtc` keeps its single meaning of real agent output. The loop previously refreshed the heartbeat on a shorter interval than the stall threshold it is measured against, which meant no captain could ever be detected as stalled; upstream's `UpdateProcessAliveAsync` supplies the split, and the fork keeps the mission-side heartbeat because its `WorkProduced` and assignment-age watchdogs read that timestamp (`ab512e0c`).
- **One mission-transition table.** The status-transition rules are defined once in `MissionStateMachine` and every entry point delegates to it — the MCP transition tool, the WebSocket command surface, the REST route, and the agent lifecycle handler. Five copies had drifted apart: two omitted every `PullRequestOpen` transition and so rejected the fork's own PR-fallback flow, and the authoritative copy lacked `WaitingForInput` entirely and classified a blocked mission as terminal. A test compares every status pair across entry points so a new copy cannot drift unnoticed (`ef84f388`).
...
- **Declared consumers built in the producer's gate.** A passing definition-of-done gate also builds every vessel that declares the mission's vessel as a sibling, so a public-API break is caught while the producer's change is still unlanded rather than surfacing on whatever builds next. The consumer edge is derived by reading the existing sibling declarations in reverse, so no vessel needs new configuration. Each consumer is materialized under a scratch root private to that verification, because a shared sibling checkout owned by another dock is reused rather than re-pointed — which would compile the consumer against a different commit than the one being reported on. Consumers are built, not tested; a consumer that cannot be prepared is reported rather than failing the producer.
...
- **Dispatch hold.** `armada_dispatch_hold` freezes all new voyage and mission dispatches — operator MCP, REST, standalone missions, and the objective scheduler — while in-flight voyages continue, so an operator working on Armada itself can rebuild and redeploy without peers starting work against a dying binary. The refusal names the holder, reason, and clearing call; engaging or clearing announces itself on the coordination board; a restart clears it deliberately so a successful redeploy resumes dispatching by itself (`de2f6773`).
- **Coordination claims and stale-peer detection.** `armada_coordination_claim` reserves a vessel or objective with a named holder and an expiry that heartbeats keep alive, so concurrent sessions reserve work instead of only posting about it. A dispatch overlapping another session's active claim proceeds but announces both parties on the board, and the Needs-You inbox raises a StalePeer warning when a silent session still holds an unexpired claim, surfacing abandoned reservations for adoption (`c6ae1192`).
- **Claims on the dashboard.** The chatroom renders active reservations as amber chips above the message stream - holder, subject, time-to-expiry - backed by `GET /api/v1/coordination/claims` and refreshed on board activity (`c5c7face`).
...
- **Consolidated navigation over hubs.** The sidebar is fourteen upstream destinations plus Code Index and Notifications, with the pages they replaced reachable as tabs and their old paths kept as redirects. `navConfig` is the single source for the sidebar and the Cmd-K palette, and a nav-inventory test pins it. Notifications is kept deliberately: upstream folds it into the inbox and its bell links to source records, which leaves that page routed but unreachable (`378e15b1`).
- **Touchstone test harness beside the fork's suites.** Upstream's shared descriptors run under the Touchstone console, xUnit, and NUnit runners with reflection-based discovery, which removes the class of bug where an unregistered test never runs. It is additive: `test/Armada.Test.*` is unchanged. Seven upstream suites are not carried because they assert APIs this fork superseded, each already covered by the fork's own tests (`3e5fa037`).
- **Captain chat and vessel Model Context.** A captain can be chatted with directly from the dashboard, with per-turn timing and token metrics, and a vessel's Model Context can be built or refined by launching a captain in a throwaway worktree. Both rest on `OnStdoutReceived`, a stdout-only runtime event added so an interactive reply does not pick up the CLI stderr banners that `OnOutputReceived` deliberately carries for missions. `ShowThinking` is honored by the Mux runtime via `--show-thinking`; every other runtime lacks a headless reasoning channel and ignores it (`c5bab62b`).
- **Telemetry export.** `ArmadaMetrics` emits the meters and `ArmadaTelemetryHost` observes and exports them over OTLP, an in-process Prometheus scrape endpoint, and Loki. Disabled by default, so a fresh install ships no telemetry surface until `telemetry.enabled` is set (`10678c39`).
- **Operator surfaces for the newer entities.** Project profiles, skills, background jobs, and the bucketed token-usage summary each have a dashboard page and REST client. The summary's builder and model already existed and were reachable only through the `token_usage_summary` MCP tool, so `GET /api/v1/token-usage/summary` exposes the same builder rather than adding a second one; it sits beside the older flat `events/token-usage` rollup, which is a different shape for a different caller (`22a5d648`).
...
- **Project-profile skills and persona overrides delivered to the captain.** A vessel's project profile (vessel then fleet then global) contributes a Skills section to the brief and can swap the prompt template for a given persona. Both resolvers are best-effort and degrade to no section. The skills block is tracked in the prompt-budget ledger like every other module, because an untracked section still costs the captain its bytes; a read-only mission takes no persona override, since its mode-aware output contract deliberately replaces the producing persona template it would customize (`f2d5d501`). Project profiles, skills, and coordination leases are implemented and wired on all four providers, and a test constructs every provider's driver offline to assert no method set is left unassigned. That check previously covered SQLite alone, which is how SQL Server came to carry a coordination-lease implementation its driver never constructed and PostgreSQL none of the three at all (`faceddbf`, `eadd5ee7`).
...
- **Bounded check-run tool output.** `run_check`, `retry_check_run`, and `get_check_run` return status, exit code, parsed test and coverage totals, artifacts, and the tail of the command output rather than the whole log. A real vessel's build or test log runs to megabytes, which overran the tool output limit and returned a truncation error in place of the verdict, forcing an out-of-band parse on every call. A truncated view reports the full log's size and names the call that fetches it (`get_check_run` with `includeOutput=true`), so nothing is withheld silently; the REST check endpoints are unchanged.
- **Expanded MCP surface.** The built-in catalog has 186 operator tool names for planning, code-index retrieval, operational assets, reflection memory, Checks, delivery, incidents, audits, Architect decomposition, AgentWake, long-running jobs, objective scheduling, captain diagnostics, coordination, and unlanded branches. Normal discovery fits on one compatibility page, while larger extension catalogs retain standard cursor pagination.
