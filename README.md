...
- **Reflections pipeline (v1 — Memory Consolidation).** Post-mission reflection system. When a vessel accumulates enough completed missions (default 15, per-vessel override via `ReflectionThreshold`), Armada auto-dispatches a `MemoryConsolidator` reflection captain that reviews the aggregate diff of all missions since the last accepted reflection, producing capsule proposals and learned playbooks. `armada_consolidate_memory` MCP tool for manual trigger. `armada_accept_memory_proposal` / `armada_reject_memory_proposal` for orchestrated review. Auto-dispatch runs during audit-drain idle cycles. Schema v40. (`30855c67`, `7266a77a`, `6f2f01ff`, `aacd963c`, `98e2c739`, `f4000735`, `a6d9126c`)
- **Reflections v2-F4 (reorganize + dualJudge).** Reorganize threshold per-vessel (`ReorganizeThreshold`), mode-aware reflection briefs, extended `armada_consolidate_memory` mode parameter, reorganize soft-validation via dual-Judge gate with quality metrics, and anti-thrash dampening (`ReorganizeAntiThrashMinSpacingHours`, `ReorganizeAntiThrashGrowthRatio`). Schema v41. (`56063c33`, `86e5efbf`, `465a687a`, `dffd0bb9`, `1f760248`, `b75053b0`)
...
- **Reflections v2-F1 (pack-curate).** `vessel_pack_hints` table and `PackCurateThreshold` per vessel. `PackUsageMiner` scans agent output for frequently-referenced code packs; dispatches a pack-curate captain via `armada_context_pack` pre-selection pass. Extended `memory_consolidator` persona with pack-curate knowledge, and `armada_accept_tool` validates pack hints with conflict detection and fan-out limits. Schema v43. (`de0b3567`, `9c223800`, `29ccadd4`, `cc2067ad`, `7286cf22`, `c43c35b2`, `ab96624d`)
- **Reflections v2-F2 (identity-curate).** Identity-memory columns on personas and captains (`learned_playbooks`, `identity_last_reflected_utc`, `captain_hash_identity`). `HabitPatternMiner` aggregates cross-vessel behavior patterns. Persona-learned playbooks bootstrap, three-way merge in `PersistMissionPlaybooksAsync`, and fan-out limits for identity-curate dispatch. Schema v44. (`5b45b421`, `7566191f`, `ecec69f4`, `3a4775ee`, `4ba68598`, `0b90da99`, `a865e366`, `1bec855f`, `b3188c23`)
- **Reflections v2-F3 (fleet-curate).** Fleet-memory schema (`learned_playbooks`, `fleet_last_reflected_utc`, `aggregate_evidence_window`). Extended `MemoryConsolidator` for seven reflection modes. Cross-vessel suggestion hints in vessel-curate briefs — when a fact from one vessel's reflection would benefit sister vessels in the same fleet, the hint is injected as a suggestion. Four-way merge collision resolution for fleet-wide learned assets. Schema v45. (`9096c102`, `0f816b50`, `0b94a1d0`, `0bfa999a`, `eb7c5f86`, `38c20f0f`, `bace72d7`, `eec40778`)
- **DeepSeek embedding/inference clients + CodeIndex V2 hybrid search.** `DeepSeekEmbeddingClient` (implements `IEmbeddingClient`) and `DeepSeekInferenceClient` (implements `IInferenceClient`) wired into `CodeIndexService` for hybrid embedding+lexical search. `CodeIndexSettings.UseSemanticSearch` (default `false`) activates semantic score blending with configurable `SemanticWeight`/`LexicalWeight`. Summarizer (`UseSummarizer`, default `false`) and file-signature (`UseFileSignatures`, default `false`) also supported but disabled by default. (`d242cc7b`, `c2ec68f1`, `86724a1d`, `2883cb21`)
...
- **v1 — Memory consolidation.** Per-vessel `ReflectionThreshold` (default 15 completed missions) triggers auto-dispatch of a `MemoryConsolidator` captain. The reflection captain reviews the aggregate diff of all missions since the last accepted reflection and produces capsule proposals and learned playbooks. `armada_consolidate_memory` MCP tool for manual triggers. Proposals accepted via `armada_accept_memory_proposal` / rejected via `armada_reject_memory_proposal`. Auto-dispatch runs during audit-drain idle cycles. Schema v40.
- **v2-F4 — Reorganize + dualJudge.** Vessel-level `ReorganizeThreshold` and `ReorganizeTokenBudget` for reorganize-only reflection modes. `armada_consolidate_memory` extended with mode parameter and dualJudge flag. Dual-Judge gate validates reorganize proposals with quality metrics before acceptance. Anti-thrash dampening via `ReorganizeAntiThrashMinSpacingHours` and `ReorganizeAntiThrashGrowthRatio`. Schema v41.
- **v2-F1 — Pack-curate.** `armada_context_pack` pre-selection pass identifies frequently-used code packs. `PackUsageMiner` scans agent output for pack references; dispatches pack-curate captains to consolidate patterns. Accept-time conflict detection checks vessel pack hints + fleet-level pack frequency for collision detection. Fan-out limited via `PackCurateFanOutMaxVessels`. Schema v43.
- **v2-F2 — Identity-curate (persona/captain).** Persona and captain identity-memory stores `learned_playbooks`, `identity_last_reflected_utc`, and `captain_hash_identity`. `HabitPatternMiner` aggregates cross-vessel behavior patterns. Three-way merge in `PersistMissionPlaybooksAsync` resolves persona-learned, captain-learned, and vessel-default playbook layers. Schema v44.
- **v2-F3 — Fleet-curate.** Fleet-memory stores fleet-level `learned_playbooks` and `aggregate_evidence_window`. Cross-vessel suggestion hints inject relevant facts from one vessel's reflection into sister-vessel curate briefs. Four-way merge collision resolution for fleet-wide learned assets. `FleetCurateFanOutMaxFleets` limits fan-out. Schema v45.
