...
- **Host-wide command serialization.** The in-dock build/test gate holds a dock lease across a host-wide queue shared by all four callers that run a vessel's build and test commands — mission gates, check runs, pending checks executed at the Judge stage, and merge-queue test runs — so they run one at a time instead of crashing the test host. A check submitted during a gate queues rather than racing, so a slow check is an expected outcome and not a symptom.
- **Checks armed on every dispatch.** Every dispatched voyage carries Build and UnitTest checks, not just rescue voyages, so a voyage cannot reach its Judge stage bare — a Judge PASS is rejected without a green independent check, so a bare voyage was already condemned when it started and nothing said so until the whole pipeline had run. Cancelling a voyage discards its checks, so a re-dispatch previously started bare again. The checks are armed Pending and executed in place at the Judge stage, so arming does not load the host as the first captain starts; the older rescue path still executes its two immediately. A type is armed only when the profile defines its command, and never twice for one voyage.
